...
func _connect_signals() -> void:
	...
	if event_bus == null:
		return
	for signal_name in ["npc_state_changed", "horse_state_changed", "building_state_changed"]:
...
func _on_authority_state_changed(_arg1: Variant = null, _arg2: Variant = null) -> void:
	_queue_refresh()
...
func _add_desired_loop(desired: Dictionary, semantic_key: String, asset_id: String, entry: Dictionary) -> void:
...
func _reconcile_loops(desired: Dictionary) -> void:
	...
	for raw_semantic_key in desired.keys():
		...
		if str(existing.get("asset_id", "")) == asset_id and audio_manager.is_loop_active(str(existing.get("loop_key", ""))):
...
func _play_entry_one_shot(asset_id: String, entry: Dictionary, source_key: String) -> void:
	...
	audio_manager.play_3d(asset_id, source, Vector3.ZERO, &"Work")
