...
func _run() -> void:
	...
	if not await _capture(Vector2i(1920, 1080), 0.0, "t0360_menu_cover_outward_facing_1920x1080_phase_a.png"):
		return
	if not await _capture(Vector2i(1920, 1080), 3.0, "t0360_menu_cover_outward_facing_1920x1080_phase_b.png"):
		return
	if not await _capture(Vector2i(1600, 1200), 3.0, "t0360_menu_cover_outward_facing_1600x1200.png"):
		return
	if not await _capture(Vector2i(2560, 1080), 3.0, "t0360_menu_cover_outward_facing_2560x1080.png"):
		return
	...
	for _frame in range(20):
		await process_frame
	print("T0360_MENU_COVER_OUTWARD_FACING_CAPTURE_PASS")
	quit(0)
