...
func _init() -> void:
	...
	if [hall, art, fixture_root, building_system, npc_system, layout_controller].has(null):
		_fail("T0131-P4 dining hall runtime hierarchy is incomplete")
		return
	if not _verify_seated_anchor_offsets(layout_controller, fixture_root, "dining_hall", "dining_seat", 10):
		return
...
func _verify_seated_anchor_offsets(controller: Node, fixture_root: Node, building_id: String, workstation_prefix: String, count: int) -> bool:
	for index in range(1, count + 1):
		...
		if (
			...
			_fail("Dining seat did not preserve the approved forward sitting offset: %s / %s" % [workstation_id, JSON.stringify(route)])
			return false
	return true
