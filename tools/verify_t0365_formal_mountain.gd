extends SceneTree
## Approved cliff appearance in actual Main, with simulation startup disabled.
const OUTPUT := "res://artifacts/visual_qa/t0365_integration"
var _metrics: Array = []
...
func _run() -> void:
	...
	var art := formal.get_node("FormalEnvironmentArtView/ApprovedStylizedMountain")
	var snapshot: Dictionary = art.verify_geometry_contract()
	assert(snapshot.enabled and snapshot.original_meshes_unchanged == 60)
	assert(snapshot.trees == 7393 and snapshot.outcrop_pieces == 21)
	var nav := formal.get_node("SpatialContract/StationNavigation") as NavigationRegion3D
	var max_navigation_x := -INF
	for v in nav.navigation_mesh.get_vertices():
		max_navigation_x = maxf(max_navigation_x, formal.to_local(nav.to_global(v)).x)
	var min_mountain_x := INF
	for surface: MeshInstance3D in art._surfaces:
		for v in surface.mesh.surface_get_arrays(0)[Mesh.ARRAY_VERTEX]:
			min_mountain_x = minf(min_mountain_x, v.x)
	assert(max_navigation_x < min_mountain_x, "The new cliffs overlap navigable ground")
	snapshot["max_navigation_x"] = max_navigation_x
	snapshot["min_mountain_x"] = min_mountain_x
	DirAccess.make_dir_recursive_absolute(OUTPUT)
	var file := FileAccess.open(OUTPUT + "/snapshot.json", FileAccess.WRITE)
	file.store_string(JSON.stringify(snapshot, "  "))
	print("T0365_INTEGRATION_PASS ", JSON.stringify(snapshot))
...
func _capture(main: Node, formal: Node3D, art: Node3D) -> void:
	...
	for entry in [
		...
		["east_wall", Vector3(65, 4, 35), Vector3(-32, 36, 50)],
		["cliff_close", Vector3(143, 14, 38), Vector3(-38, 22, 34)],
		["cliff_overview", Vector3(185, 28, 112), Vector3(-110, 105, 110)],
		["cliff_side", Vector3(173, 24, 100), Vector3(-88, 32, -64)],
		...
		for night in [false, true]:
			...
			assert(result == OK)
		if "--measure-mountain" in OS.get_cmdline_user_args() and entry[0] in ["station", "cliff_overview", "full_map"]:
			time_system.set_current_time(3, 12, 30, 0)
			for enabled in [false, true]:
				art.set_enabled(enabled)
				await _measure(str(entry[0]), enabled)
	print("T0365_INTEGRATION_CAPTURES ", OUTPUT)
	if not _metrics.is_empty():
		...
		print("T0365_RENDER_METRICS ", JSON.stringify(_metrics))
