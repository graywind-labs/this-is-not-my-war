...
func _init() -> void:
	...
	if int(snapshot.get("configuration_error_count",-1))!=0 or int(snapshot.get("natural_boundary_count",0))!=24 or int(physics.get("natural_boundary_count",0))!=24 or int(physics.get("static_body_count",0))!=234 or int(production.get("source_static_body_count",0))!=234 or int(production.get("vertex_count",0))!=788 or int(production.get("polygon_count",0))!=754:_fail("A3 final counts/config drifted: %s / %s / %s"%[snapshot,physics,production]);return
