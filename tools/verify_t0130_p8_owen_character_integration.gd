...
func _init() -> void:
	...
	var left_pouch_position := Vector3(initial.get("engineer_tool_pouch_left_position", Vector3.ZERO))
	var right_pouch_position := Vector3(initial.get("engineer_tool_pouch_right_position", Vector3.ZERO))
	check(absf(left_pouch_position.x) <= 0.23 and absf(right_pouch_position.x) <= 0.23, "Owen's square tool pouches still float away from his waist sides")
	check(left_pouch_position.z <= 0.11 and right_pouch_position.z <= 0.11, "Owen's square tool pouches still float too far in front of his waist")
	check(str(initial.get("engineer_folding_rule_parent", "")) == "ToolPouchLeft", "Owen's folding rule should follow the left waist pouch")
	check(str(initial.get("engineer_wood_wedge_parent", "")) == "ToolPouchRight", "Owen's triangular tool should follow the right waist pouch")
	check(Vector3(initial.get("engineer_folding_rule_local_position", Vector3.ONE)).length() < 0.14, "Owen's folding rule still floats away from its pouch")
	check(Vector3(initial.get("engineer_wood_wedge_local_position", Vector3.ONE)).length() < 0.12, "Owen's triangular tool still floats away from its pouch")
	...
	for state_name in expected_states.keys():
		...
		if state_name == "work":
			check(float(preview.get("engineer_wrench_grip_hand_distance", 1.0)) < 0.01, "Owen wrench handle end is not seated in his hand")
			check(float(preview.get("engineer_wrench_forward_dot", -1.0)) > 0.9, "Owen wrench jaws do not point forward")
			check(float(preview.get("engineer_wrench_down_dot", -1.0)) > 0.1, "Owen wrench is not angled toward his assembly work")
