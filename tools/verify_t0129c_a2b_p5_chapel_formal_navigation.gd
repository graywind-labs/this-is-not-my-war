...
const PILOT_ID := "chapel_prayer_seat"
const NPC_ID := "priest_01"
const BUILDING_ID := "chapel"
const FIRST_SEAT_ID := "chapel_prayer_seat_01"
const SECOND_SEAT_ID := "chapel_prayer_seat_02"
...
func _init() -> void:
	...
	_npc = root.get_node_or_null("Main/WorldRoot/Station/NPCs/Priest01") as CharacterBody3D
	if npc_system == null or _building_system == null or _npc == null or event_bus == null:
		_fail("A2b-P5 runtime dependencies unavailable")
		return
	...
	for required_id in ["glen", "clinic_doctor", "clinic_bed", "dormitory_bed", "dining_seat", PILOT_ID]:
	...
	if not _validate_start_result(start_result, FIRST_SEAT_ID):
		_fail("Chapel pilot did not select the first free prayer seat: %s" % start_result)
		return
	if not await _wait_for_phase(npc_system, "occupant_attached", FIRST_SEAT_ID):
		_fail("Marcel did not reach and attach to the first prayer seat: %s" % npc_system.debug_get_chapel_navigation_pilot_snapshot())
		return
	await process_frame
	var first_snapshot: Dictionary = npc_system.debug_get_chapel_navigation_pilot_snapshot()
	...
	if (
		...
		or str(first_attachment.get("pose", "")) != "seated_prayer"
		...
		_fail("Prayer-seat commit / seated-prayer attachment / no-prayer-action contract failed: %s" % first_snapshot)
		return
	...
	if not _workstation_is_clear(FIRST_SEAT_ID) or bool(_npc.debug_get_spatial_attachment_snapshot().get("active", false)):
		_fail("Explicit stop left a prayer-seat claim or attachment")
		return

	var blocker_reservation: Dictionary = _building_system.reserve_workstation(BUILDING_ID, BLOCKER_NPC_ID, "chapel_prayer_seat")
	if str(blocker_reservation.get("workstation_id", "")) != FIRST_SEAT_ID:
		_fail("Could not reserve chapel_prayer_seat_01 for first-free selection test: %s" % blocker_reservation)
		return
	var blocker_commit: Dictionary = _building_system.commit_workstation_reservation(BUILDING_ID, BLOCKER_NPC_ID, FIRST_SEAT_ID, "chapel_prayer_seat")
	if not bool(blocker_commit.get("ok", false)):
		_fail("Could not occupy chapel_prayer_seat_01 for first-free selection test: %s" % blocker_commit)
		return
	...
	if not _validate_start_result(second_start, SECOND_SEAT_ID):
		_fail("Chapel pilot did not skip occupied seat 01 and select seat 02: %s" % second_start)
		return
	if not await _wait_for_phase(npc_system, "occupant_attached", SECOND_SEAT_ID):
		_fail("Marcel did not attach to the second free prayer seat")
		return
	...
	if not _workstation_owned_by(FIRST_SEAT_ID, BLOCKER_NPC_ID) or not _workstation_is_clear(SECOND_SEAT_ID):
		_fail("Pilot cleanup altered another NPC's prayer seat or left seat 02 occupied")
		return
	...
	if not bool(failure_start.get("ok", false)):
		_fail("Chapel unreachable-path pilot did not start: %s" % failure_start)
		return
	await physics_frame
	if not _npc.request_motion(Vector3(900.0, _npc.global_position.y, 60.0), "forced_chapel_unreachable"):
		_fail("Could not inject an unreachable chapel target")
		return
	if not await _wait_for_phase(npc_system, "navigation_failed", "", 720):
		_fail("Unreachable chapel target did not settle")
		return
	if not _workstation_is_clear(FIRST_SEAT_ID) or bool(_npc.debug_get_spatial_attachment_snapshot().get("active", false)):
		_fail("Unreachable chapel target left a reservation or attachment")
		return
	...
	if not bool(supersede_start.get("ok", false)) or not await _wait_for_phase(npc_system, "occupant_attached", FIRST_SEAT_ID):
		_fail("Chapel supersession pilot did not reach seat 01")
		return
	...
	if (
		...
		or bool(npc_system.debug_get_chapel_navigation_pilot_snapshot().get("active", false))
	):
		_fail("Data-driven pilot supersession did not atomically restore Marcel")
		return
	...
	if not bool(unavailable_start.get("ok", false)) or not await _wait_for_phase(npc_system, "occupant_attached", FIRST_SEAT_ID):
		_fail("Chapel building-unavailable pilot did not reach seat 01")
		return
	var chapel_before_damage: Dictionary = _building_system.get_building(BUILDING_ID)
	var max_hp := int(chapel_before_damage.get("max_hp", 100))
	_building_system.debug_damage_building(BUILDING_ID, max_hp)
	await process_frame
	if (
		bool(npc_system.debug_get_chapel_navigation_pilot_snapshot().get("active", false))
		or not _workstation_is_clear(FIRST_SEAT_ID)
		or bool(_npc.debug_get_spatial_attachment_snapshot().get("active", false))
	):
		_fail("Building unavailability did not clear Marcel's chapel pilot")
		return
	...
	if not bool(unconscious_start.get("ok", false)) or not await _wait_for_phase(npc_system, "occupant_attached", FIRST_SEAT_ID):
		_fail("Chapel unconscious-path pilot did not reach seat 01")
		return
	...
	if (
		...
		_fail("Unconscious interruption did not atomically clear Marcel's chapel pilot")
		return

	print("T0129C A2b-P5 chapel formal navigation verification passed: %s" % JSON.stringify({
		...
		"occupant_pose": "seated_prayer",
		"prayer_action_started": false,
		"stop_cleanup": true,
...
func _validate_start_result(result: Dictionary, expected_seat_id: String) -> bool:
	return (
		...
		and str(result.get("workstation_type", "")) == "chapel_prayer_seat"
		and str(result.get("expected_workstation_id", "")) == ""
		and str(result.get("arrival_mode", "")) == "mount_after_arrival"
		and str(result.get("occupant_pose", "")) == "seated_prayer"
	)
