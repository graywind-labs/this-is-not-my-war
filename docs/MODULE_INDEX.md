...
当前状态：T0703 后已包含资源、时间、建筑、NPC、记忆与地点信号，以及 `recruitment_changed`、`npc_order_changed`、`npc_plan_reevaluation_requested`；指令变化与计划重评估请求可由后续计划系统独立监听。T1302 后新增 `game_over_changed(result, reason)`，用于主厅失败等结算状态广播。
