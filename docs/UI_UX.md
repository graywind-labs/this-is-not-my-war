...
- `BuildingPanel` 内的修复、升级按钮会调用 `BuildingSystem`；按钮根据当前 HP、等级、配置、是否已有修复作业和资源是否足够自动启用或禁用。光标悬停在修复/升级按钮上时，按钮旁边显示操作所需资源和条件提示框。修复进行中会显示进度、剩余时间、速度倍率和协助人数。
...
- 当前工作位
- 当前地点信息占位
- 修复按钮，资源足够、建筑受损且没有正在修复时可用；修复开始后显示倒计时状态
- 升级按钮，资源足够且未达最高等级时可用
...
- 修复/升级按钮可触发最小升级结算和倒计时修复；资源不足、条件不满足或已有修复作业时按钮禁用。修复/升级消耗和可执行条件只在按钮悬停提示框中显示，不写入面板正文。
- 当前不显示真实产出、储存或完整升级树。
