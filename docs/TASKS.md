...
## GPT6 3D 美术表现升级试验系列（2026-09-05）

### T0396 完整 Main 多人战斗性能优化（机制冻结）

状态：Partial（2026-09-09；已按“GM 全员一键入伍/配装 → GM 第三波”精确链路完成第二轮收口，约 15→26.7 FPS，但仍未达到稳定 60 FPS）

用户要求：第三波起多人战斗严重卡顿；制定专业、完整、不改变游戏机制且尽量降低回归风险的方案，专门写入 Markdown 后实施。

实施与验收以 `docs/COMBAT_PERFORMANCE_OPTIMIZATION_PLAN.md` 为准。冻结敌人 8/16/24/36/48、波次时间、战斗参数、AI 决策频率、攻击/弹体/碰撞权威和事件语义；先建立完整 Main 与热点对照，优先消除重复快照、重复表现刷新及无变化状态写入。寻路频率、物理/RVO 参数、碰撞与 T0234 实体拥堵属于行为敏感项，不凭猜测顺带调整。每阶段独立测量、回归和记录；未达到完整场景性能目标不得写成全量达标。

已落地：无效全候选预验路删除、同步局部 body/空间索引、只读轻量投影、NPC 状态提交与通用信号消费者相关性短路、建筑不可变静态 Box/单表面网格精确合批、远程攻击位等值提前结束、方向光动态阴影 `4→2` 级联，以及默认关闭的限时采样、GM 入口、完整 Main 基准和断网回归工具。方向光仍保持唯一日/月阴影、120m 范围、边界混合与全部投影者；未关闭敌人/建筑阴影。

精确结果：正式 Main、D3D12 1280×720、GM 保持打开、8 名 NPC 全员入伍/配装、GM 第三波、24 秒预热+10 秒真实交战样本，第二轮起点帧均值/P50/P95 约 `66.99/64.60/90.20 ms`，最终为 `37.45/33.92/54.67 ms`，Draw Calls 约 `11,468→4,225`；最终记录 44 次攻击挥动、23 次投射物释放、22 次命中、56 次角色受伤，模型请求 0。帧时间下降约 44%，但 P95 仍未达 60 FPS，物理/战斗逻辑均值约 `17.82/9.44 ms`。第一轮自然进场 `27.54 ms` 与玩家精确 GM 链路不是同一交战夹具，不再用于声称玩家问题已修复。

验收与风险：8,118 项等值检查、P6/P6R/P6R2/P6R3/P7R、攻击时间线、引导区、近战稳定、屋顶、建筑升级和主厅切片通过。全量 GM 回归在与性能入口无关的既有训练教官到岗断言失败，未篡改旧测试。下一阶段关卡是保持接触/碰撞/RVO/命中完全等值的物理查询时点迁移和锁步回放；独立物理线程的直接实验因 `Space state inaccessible` 已撤回，不能直接开启。T0234 拥堵继续独立。

防重复要求：`docs/COMBAT_PERFORMANCE_OPTIMIZATION_PLAN.md` 第 11 节是 T0396 性能方向权威台账，已登记所有保留、无收益撤回、仅诊断禁用及待架构化方向。后续任何性能实验都必须先查台账，并将成立/不成立、原始夹具、指标和回滚状态回写；只有引擎/资产/架构/夹具实质变化或新 profiler 证据才允许重访失败方向。

### T0395 六类音频归档与七项音量设置

状态：Done（2026-09-09）

按用户确认方案将全部 102 条正式音频归入音乐、点击、语气、战斗、工作、环境六类，设置页连同主音量提供七个实际生效的滑杆。音乐默认及旧配置实际值降为原先 50%；升级 / 修复点击、铁匠铺和工械坊制造目标选择统一为不衰减点击声；奔跑、陨石、警报与受击归战斗，门和行商归环境。移除玩家可见的笼统“音效”控制，并完成旧五项配置的一次性迁移。

完成：新增 102 条全覆盖、无重复的配置化分类表，正式设置页显示七个滑杆。六个分类总线分别直达 Master；AmbientBed 继续隶属环境，旧 Foley 内部总线汇入战斗，旧 SFX 保留兼容但不再承载玩家分类音量。音乐新默认 28%，v1 设置迁移为音乐减半、旧 SFX 拆给语气 / 战斗 / 工作、旧 UI 继承给点击。全套 P10A/C/D/E/F/G/H、T0380、T0393、T0354 与 Main headless 通过；MCP 正式前端读取到七项控件和 `12/7/19/33/16/15=102` 分类。版本 `0.0.694-t0395-audio-categories`。

### T0394 建筑施工与铁匠交互音量增强

状态：Done（2026-09-09）

用户反馈：修复 / 升级按钮的短木击、施工期间的持续敲打、铁匠铺选定制造目标三类声音相较其他音效偏小。仅提高这三类已确认反馈的独立播放增益；施工循环继续从建筑位置播放并保留既有距离衰减，不改变其他工作声、UI 声和总线音量。

完成：三类事件均增加独立 `+4 dB` 播放增益；AudioManager 的 3D 单次与循环入口支持显式事件增益，循环渐入目标由固定 `0 dB` 改为配置值。T0393、P10A、P10C、P10G 和 Main headless 通过；MCP 正式 Main 验证修复开始、施工循环、铁匠选择均读取 `+4 dB`，位置档仍为 `12m / 112m`。版本 `0.0.693-t0394-building-audio-gain`。

### T0393 建筑施工与铁匠目标确认音频接入

状态：Done（2026-09-09）

用户确认：修复 / 升级成功开始使用 v27 短木击 05；建筑处于修复 / 升级期间使用 v27 施工循环 01，从建筑真实位置播放并随镜头距离衰减，开停渐入渐出；升级完成复用既有 NPC 升级成功声；铁匠铺成功选定非空制造目标时使用 v27 敲铁 04。失败请求、禁用按钮、程序刷新及未选择目标不得冒充成功发声。

完成：新增两条 48 kHz 单声道正式 WAV，资产总数增至 102；既有施工 OGG 继续复用。WorkAudioController 现以 BuildingSystem 活动作业为施工循环唯一补充依据，无助手也从建筑入口位置播放，使用本地 3D `unit_size=12m / max_distance=112m` 和 350ms 启停渐变；作业从空切到 repair / upgrade 时播放短木击且同按钮不叠普通 UI 点击。铁匠铺制造目标只有真实变为非空新值时播放敲铁，重复、清空、取消、非法目标、刷新和工械坊静音。InteractionAudioController 仅在 upgrade 完成时复用 level_up，repair 完成不播。T0393、P10A、P10C、P10G、T0205、102 条资产 QA 与 Main headless 通过；Godot MCP 正式 Main 实测修复开始、位置循环、铁匠选择和升级完成全链。版本 `0.0.692-t0393-building-craft-audio`。

### T0392 守备官攻击 NPC 强制受击叫声

状态：Done（2026-09-08）

用户要求：守备官攻击 NPC 时除必播打击命中声外，还必须按 NPC 性别 100% 播放已经确认的男女受击叫声；普通战斗中友方 NPC 受击仍维持原确认的 50% 概率。只调整表现层概率判定，不改变伤害、HP、昏迷、记忆、对话或战斗权威。

完成：`NPCSystem` 已有伤害事件携带真实 `source_id=guard_officer`，`CombatAudioController` 现仅对该来源把受击叫声概率覆盖为 100%，并继续按目标 NPC 性别选择已确认的男 / 女语音；其他来源仍读取通用 50%。P10E 覆盖配置、男女映射、真实 `debug_damage_npc` 权威链和非守备官对照；对话生命周期与 Main headless 通过。Godot MCP 正式 Main 对男女 NPC 各实际攻击一次，均同时记录一条随机打击声和正确性别叫声。版本 `0.0.691-t0392-guard-hurt-voice`。

### T0391 全建筑施工木质脚手架

状态：Done（2026-09-08）

用户要求：每个建筑升级或修复时紧贴旁边显示木质脚手架，完工或施工被打断时消失。为全部 15 类建筑分别配置贴边位置、跨度、层数与木构细节，复用正式 3D 美术风格。只读 BuildingSystem 作业状态；不改施工权威、NPC 行动、碰撞、导航和音频。验收覆盖全部建筑、修复/升级开始与完成、受击中断、重复施工及实际正侧面渲染；直接在 Main 可见，复用现有 GM 施工/伤害入口。

完成：新增配置化 BuildingScaffold，按需构建三组 MultiMesh 木框 / 板台靠梯 / 绳绑；普通建筑为分跨侧架，菜园和训练场为矮架，门楼双塔架留中央通道，围墙贴真实墙段旋转。仓库施工架在西侧地台，避开二级东侧扩建；排除旧隐藏铁匠铺样例，保证 15 类各一个控制节点。施工结束同步隐藏并释放几何，同帧重新开工不残留。只减员而未取消工程时仍显示。

验证：全 15 类修复 / 升级开始、完成、受击中断、重启、逐级升级至满级和满级修复通过；D3D12 实际渲染同专项通过，初级 / 满级正侧面共 60 图已检查。T0205 原修复 / 升级回归通过（该旧脚本退出有 ObjectDB 泄漏警告；新专项退出无此警告）。Godot MCP 正式 Main 读取到 15 个脚手架控制节点，主厅开工 / 受击同步切换通过，编辑器错误日志为空。网格诊断发现既有导入屋顶 / 盾牌 / 铁砧退化 UV 和草片绕序提示，未修改这些旧资产；脚手架使用原生 BoxMesh。截图和日志位于 artifacts/visual_qa/t0391/ 与 artifacts/t0391_*.log。无 LLM 测试需求，无新增 GM 按钮。

### T0389 自然环境底噪替换与野生动物声密度收敛

状态：Done（2026-09-08）

用户反馈（2026-09-08）：现白天全局底噪鸟叫过密且含游戏中不存在的羊叫，夜间全局底噪虫鸣过响，均需替换；黎明鸡叫每个游戏日只触发一次；三条白天鸟叫和三条夜间虫鸣改为位置化间歇触发并降低响度。

范围：替换两条全局昼夜环境底噪及其来源登记；保持底噪随镜头缩放但不绑定地点的既有合同。将夜间三条常驻虫鸣从循环发声器迁入随机组，与既有白天鸟叫一样按“完整播放一条 → 静默间隔 → 再选下一条”调度；鸟鸣和虫鸣分别配置独立较低增益。为黎明鸡叫增加按游戏日计数的一次性上限，并覆盖跨日、黎明边界、随机间隔、禁止同组重叠和环境总线归属。只改表现层，不改变时间、NPC、动物、环境或战斗权威。

阶段完成：白天鸟叫保持三位置池但改为每条完整结束后静默 55–110 秒，并统一降低 `7 dB`；夜间三条虫鸣从整夜常驻循环迁为单一随机池，每条完整结束后静默 35–75 秒，并统一降低 `8 dB`。`AudioManager` 的 one-shot 入口现显式关闭源文件循环标记，避免循环型 OGG 被间歇播放时永久占用。鸡鸣以游戏日为键限制一次，并在 05:00 / 06:00 / 08:00 黎明与昼夜边界重建调度时保留当天计数。P10A、P10B 和 Main headless 通过。

底噪选材完成：v25 纯风方向已整体否决；用户从 `sample_pack_v26_ambient_nature_beds` 确认白天 04 `ambience birds loop.wav`、夜晚 01 `Quiet Night Atmosphere – Soft Crickets`。两条均采用实际试听的前 60 秒，并将末尾 2 秒与开头 2 秒交叉衔接为 58 秒无缝循环母源；正式运行时保持单声道 48 kHz Vorbis，分别以 `-28 / -30 LUFS` 接入原 `sfx_ambience_day_loop / sfx_ambience_night_loop` ID，因此既有全局 AmbientBed、昼夜切换、1 秒启停淡化及镜头缩放 `1.0 → 0.3` 非零响度合同不变。旧昼夜底噪改为 rejected，来源、母版、运行时、清单与信用表已同步。P10A、P10B、Main headless 与 100 条资产 QA 通过；Godot MCP 正式 Main 运行态确认白天加载 v26-04，切至 19:00 后唯一全局底噪切换为 v26-01。版本 `0.0.690-t0389-ambient-beds`。

### T0388 胜负结局面板视觉与信息层级重构

状态：Done（2026-09-08）

用户要求（2026-09-08）：结局面板按胜负使用两套明显不同的美术主题；胜利更华丽明亮，失败更灰暗克制。顶部使用大号风格化“胜利 / 失败”，其下只显示守住波次数；摘要只保留时间、结局标题和主题，不再显示原因、剩余资源、建筑、NPC 汇总或“NPC 结局”字样。主体纵向滚动，每名 NPC 独立分块，左侧约三分之一为人物画像，右侧只显示是否逃离、是否入伍及生成的结局故事。

实施边界：结算 UI 只读 `GameState.settlement_snapshot` 与既有群像结局，不更改胜负、波次、人物状态或 LLM 事实；画像复用正式 NPC 实时人物镜头。GM 继续使用 T0387 一键胜利 / 失败入口验收，不新增第二套胜负按钮。

完成：新增独立 `EpiloguePanel` 表现组件，提供金色暖调胜利主题与银灰低饱和失败主题；顶部字段严格收敛为时间、结局标题、主题三项，以群像 `station_coda` 投影主题。八名人物按快照顺序生成纵向卡片，复用实时 3D 画像并允许结算档案镜头继续拍摄已离站人物，右侧只保留是否逃离、是否入伍与 `fate_summary`，不显示最终位置、HP、最终看法或技术降级信息。`tools/verify_t0388_epilogue_panel.gd` 覆盖双主题、波数、三字段、八人卡片、1:2 栏宽意图、逃离者画像模式和旧字段缺席；既有胜负与 T0387 HUD 断言同步为新结构。

追加优化（2026-09-08）：按用户验收反馈把面板从近全屏收至居中的约 `2/3 × 2/3` 视口；保留结局标题和主题正文，但移除“结局标题”“主题”两个显式字段名，仅保留“时间”小标签。

### T0387 胜负结算的真实 LLM 群像结局

状态：Partial（2026-09-08 程序与 Mock 闭环已完成；真实 Provider 文学质量与未来完整存档复用待验收）

真实 Provider 补充验收（2026-09-08）：新增 `tools/verify_game_epilogue_real.py`，使用临时进程环境将已有 `LLM_API_KEY` 接入 `deepseek/deepseek-v4-flash`，不修改 `.env`，完成一组八人胜利和一组八人失败的真实 `/game/epilogue` 返回，均 `fallback=false`；失败样本首次业务校验不通过后经一次 correction 成功。合法展示批次共消费 22,810 tokens，估算 ¥0.02677。另两次包含“胜利时马塞尔已逃离”的边界验收均在一次 correction 后仍因 tone 不兼容而被 502 拒绝，其中首次还出现禁用“阵亡”措辞；这证明失败不会伪装成功，但复杂胜利人物的 tone / 纠错提示仍需加强。人工阅读还发现模型会补写“守备官亲自拖人离开火线”“新来的厨娘”等未由 fact_refs 支持的细节，且八篇较多复用“第一件事—数月后—某个时刻”句式，任务继续保持 Partial。
用户要求（2026-09-08）：胜利和失败结算都为每名 NPC 生成有故事性、符合设定、真实游戏走向和最终处境的后日谈；胜利整体倾向希望与较好归宿，失败整体倾向伤感与失去，并保持悠长、克制、令人回味的叙事感。

设计定案：
- 使用一次 `game_epilogue` 群像请求原子生成全站尾声和 8 名 NPC 结局，不拆成 8 个互不知情的请求。共享同一时间跨度、驿站战后状态和人物交集，避免某篇说驿站重建、另一篇说驿站永久废弃等连续性冲突。
- GameState 在胜负成立时先冻结不可变 `epilogue_fact_snapshot`。程序事实至少覆盖胜负、原因、结束时间、最终波次、资源和建筑状态、每名 NPC 的身份 / 职业 / 入伍 / HP / 昏迷 / 逃离 / 最后位置 / 当前指令，以及从权威事件、见闻、日记和知识图谱压缩出的关键经历。LLM 只能续写战后可能人生，不能改写这些事实。
- 增加确定性的 `EpilogueFactCompiler` 语义层：按人物筛选应征、拒绝、赠予、惩戒、指令、工作与制造、战斗、击退、受伤、昏迷、治疗、复苏、逃离 / 挽留及重要关系事件；同类高频事实聚合，保留最早、最重、最后与转折点，并给出稳定 `fact_id`。不把原始全集、内部枚举或调试事实直接交给模型。
- 叙事采用三拍结构：战后眼前的动作或处境 → 数月 / 数年后的职业与关系余波 → 一个克制的感官意象收束。每人正文建议 180–280 个中文字符，引用 1–3 个真实关键经历，但不得写成战报、属性清单或道德判词。
- 胜利是“偏暖而非人人圆满”：可行动者通常获得重建、归乡、留下或重新选择生活的机会；昏迷者仍可疗养和复苏；逃离者即使活下来也保留缺席、愧疚、释然或远方消息的复杂余味。失败是“偏伤感而非强制悲惨”：突出离散、废墟、未兑现承诺和职业理想受损，但不得宣告任何初始 NPC 死亡，也应允许微弱尊严、互助或记忆延续。
- 职业意象只作为人物专属母题，不套模板：马夫对应蹄声 / 道路，厨子对应炉火 / 面包，园丁对应种子 / 季节，铁匠对应火星 / 铁器，副官对应名册 / 旧军装，神父对应钟声 / 祷词，医生对应药草 / 伤口，工程师对应图纸 / 石缝。必须结合实际经历选择，不能八篇逐项机械使用。
- 返回结构包含 `result`、`station_coda`、`ending_title`、严格覆盖正式 NPC ID 的 `npc_endings[]`；每项包含 `npc_id`、`ending_title`、权威起点回显 `opening_status`、`final_opinion`、`fate_story`、`tone`、`fact_refs[]`。后端拒绝漏人、多造人物、事实引用不存在、胜败不匹配、NPC 死亡、状态矛盾、现代词汇和越界权威结果。
- 结算 UI 先立即显示已成立的胜负与程序摘要，再显示“战地记录整理中”；整组结果通过后原子替换当前 Mock 字段。迟到响应必须绑定本次结算 id，不能覆盖新局或读档后的结算。
- 正式 Provider 最多 2 次真实尝试（首次 + 一次 Schema / 连续性纠错）。失败、超时、无 Key 或预算不足时使用明确标记为 `template_fallback` 的确定性文学模板，不以 Mock 冒充成功；技术状态与 usage 保留 request id、call_type、provider、model、HTTP / 异常、失败原因和降级来源。
- 结局只用于结算展示和存档快照，不回写事件、见闻、日记、知识图谱或 NPC 权威状态。保存已成功生成的结局；读档后不重复付费生成。开发期 GM 提供胜利 / 失败事实夹具、请求状态、结果来源和重试入口，不另造胜负权威。

验收标准：
- Mock / fake 覆盖胜败、8 人完整性、Schema、事实引用、迟到响应、存档复用、降级来源和禁用死亡表述；真实 Provider 至少覆盖一组胜利与一组失败的完整八人群像。
- 人工叙事验收逐篇确认：至少一个可追溯的本局事实、与职业 / 人格一致、与最终状态不冲突、胜败基调正确、三拍完整、收束意象自然；8 篇不得出现显著同句式复写或互相矛盾的未来。
- 胜利样本整体温暖但允许苦涩，失败样本整体伤感但不滥用死亡、绝望或廉价反转；任何好坏倾向都不能覆盖逃离、昏迷、受伤、施暴或承诺失信等真实后果。
- UI 在请求期间可读、失败后可读、滚动和较小分辨率下可完整查看；模型调用不会恢复游戏推进，也不会改变资源、HP、建筑、胜负或 NPC 状态。
- 真实 API 未通过前任务不得标记 Done。

完成记录（2026-09-08）：已新增确定性 `EpilogueFactCompiler`、`EpilogueSystem`、`POST /game/epilogue`、严格请求 / 响应 Schema、群像 Prompt、一次业务纠错上限、LLMBridge 异步通道和 HUD 原子刷新。胜负成立后结算页先进入 `pending`，完整响应按 request id + 当前 settlement id 校验后一次性覆盖 8 人原占位字段；后端失败、非法响应或自动 Mock fallback 会转为明确的 `template_fallback`，不把 Mock 冒充正式成功。模板和模型结果都不写回 NPC 权威状态或记忆。后端胜败双分支 Mock、Godot 八人 mock 往返、断网模板降级、Main headless 与 Godot MCP 正式 Main 节点 / 编辑器错误检查通过。当前 `backend/.env` 的文本 Provider 显式为 `mock`，没有可用真实文本模型配置，因此未产生真实费用，也未完成人工文学质量验收；项目尚无完整游戏存档系统，当前只保证同一运行态结算快照复用，读档持久化留待存档系统接入时完成。

追加验收任务（2026-09-08）：用户要求在 GM 面板增加“一键胜利结算 / 一键失败结算”，用于不经过五波完整流程直接验收群像结局。入口必须调用 CombatSystem 的 `debug_*` 结果包装：胜利复用正式第五波胜利快照构造，失败先经 BuildingSystem 权威伤害把主厅降至 0，再复用正式主厅失败提交；GMPanel 不自行拼造资源、建筑或 NPC 事实。面板同时显示 `pending / llm / template_fallback`、Provider / model 或失败原因，并保留命令等价入口。

追加完成（2026-09-08）：GM“常用 → 群像结局验收”现提供“一键胜利结算 / 一键失败结算 / 刷新结局状态”。胜利调用 `CombatSystem.debug_trigger_game_outcome("victory")` 复用正式第五波结算快照；失败调用同一接口，经 BuildingSystem 把主厅真实降至 0 后复用 `_trigger_main_hall_failure`。状态行监听 `epilogue_changed`，显示生成中、LLM Provider / model 或模板降级原因；命令等价入口为 `epilogue_victory / epilogue_failure / epilogue_status`。胜利和失败按钮专项均通过。完整 GM 回归成功走过新增控件检查，后续仍在既有 `train_instructor` 真实工位断言处失败，与结局入口无关。

### T0386 五波敌袭前移与经营等价重标定

状态：Partial（实现完成；连续五波回放与真实玩家盲测待补验）
用户确认（2026-09-08）：五波来袭依次改为第 1 日 23:00、第 3 日 08:00、第 4 日 04:00、第 5 日 12:00、第 6 日 18:00；在不改变现有各波目标通关率、难度曲线和敌军战斗数值的前提下，按已推演方案系统性压缩经营准备周期。

范围与定案：
- 冻结五波敌军 `8 / 16 / 24 / 36 / 48` 的构成、HP、攻击、防御、穿透、攻速、移动、目标优先级，以及我方装备 / 器械战斗属性、建筑 HP、虔诚产出 / 陨石、恢复、需求、商人价格与征募 Prompt。
- 铁匠制造阶段基础周期 `5400 → 4800` 游戏秒；工械制造阶段基础周期 `5400 → 4500` 游戏秒。配方阶段数和完整材料总量不变。
- 新游戏工械坊预选箭塔并完成前 4 / 12 阶段；这 4 阶段已经消耗 4 木，因此松散初始木材 `12 → 8`，不追授欧文技能或总经验。资源禀赋保持 `8 松散木 + 4 已投入木 = 12`，首塔余料仍为木 4、铁 2。
- 新自然有效制造窗按第一至第五波前约 `7 / 14.8 / 21 / 31.9 / 42h` 校准；确认工作流累计工时目标为格伦 `3.573 / 3.573 / 13.161 / 31.801 / 37.727h`、欧文 `6.656 / 9.107 / 20.290 / 29.814 / 41.718h`。
- HUD 在每波 T-3h 与 T-30min 各提示一次并突出警报按钮，不自动鸣警。警报或敌军真正出现时，允许取消当前首次熟睡反思的在途请求、释放深睡锁，并把该窗口恢复为可重试；战后下一段合格睡眠继续反思，不伪造结果、不丢短期记忆。
- 更新连续五波回放的时间轴、交易提前量、后勤班次与断言；至少覆盖波次配置、制造工时、半成品资源守恒、夜袭警报 / 反思互斥、HUD 阈值提示、固定战斗快照与 Main 运行。

验收标准：运行时五波在新时点各触发一次；新游戏工械坊显示箭塔 4 / 12 且仓库木材为 8；换项目仍按既有确认规则放弃已完阶段且不返还已投入木材；夜袭不会因首次熟睡反思锁住合法战斗员；所有冻结战斗数值逐字段不变。自动化只能证明合同和样本闭合，现有目标通关率继续作为 KPI，须由后续真实玩家盲测校准，不把单次回放冒充概率证明。

完成记录（2026-09-08）：已按“文档定案在先、程序实施在后”完成五波时间、`4800 / 4500s` 阶段周期、8 松散木 + 4 已投入木、箭塔 4 / 12 初始项目、T-3h / T-30min 一次性预警、警报强调和战斗中断在途首次熟睡反思。配置 / 工时 / 资源守恒专项、T0121 合同、波次调度与 HUD、战斗警报互斥、制造管线、第五波固定战斗快照、Main headless 及 Godot MCP 正式 Main 可视验收通过；MCP 实读 W1 D1 23:00、W5 D6 18:00、木 8、箭塔 4 / 12 和两档提示，编辑器无错误。完整 T0122 压缩日历回放因运行耗时异常长未在本轮跑完；原 `verify_daily_reflection_system.gd` 与 `verify_action_system_basic.gd` 仍分别卡在既有宿舍到床 / 空间移动前置条件。因此实现已完成，但新日历下的连续经营闭环、目标通关率和真实玩家盲测仍保持待验，不能据单次固定战斗样本宣称概率不变。版本 `0.0.683-t0386-compressed-wave-balance`。

### T0385 关键建筑受击碎屑命中点修复

状态：Done
用户反馈（2026-09-07）：城门、仓库、主厅受击时，碎片没有从实际受击处溅起，而是出现在建筑上方。
范围：排查建筑伤害提交、世界飘字和 `structure_damaged` 战斗表现事件的坐标链；将结构碎屑固定到真实命中点，同时保留三座关键建筑现有的顶部 HP 飘字锚点。覆盖真实命中坐标、无命中坐标回退及 CombatVFX 最终粒子落点，不修改伤害、HP、摧毁、攻击位或建筑美术。
完成（2026-09-07）：根因是 `BuildingSystem` 为三座关键建筑强制选择屋顶上方 HP 飘字锚点后，`structure_damaged.world_position` 被 CombatVFX 直接复用并再次上抬 `0.8m`。现为结构事件增加独立 `impact_world_position`：真实近战 / 弹体命中点原样保留，无碰撞点的 GM / 系统伤害回退到地面入口；`world_position` 和 WorldFeedbackPresenter 继续保持原顶部锚点，音频与既有消费者合同不变。CombatVFX 仅对建筑碎屑优先读取新字段。T0385、T0317、T0312、T0133 与 Main headless 通过；T0184 旧城门时间轴布场仍因样本敌人全程 idle 未走到伤害阶段而失败，与本次事件坐标拆分无关。版本 `0.0.682-t0385-building-debris-impact`。

### T0384 NPC 赠酒后 GM 饮酒目标同步与行动短文案

状态：Done
用户反馈（2026-09-07）：在 NPC 面板赠酒后使用 GM 指定该 NPC 喝酒，事件库仍显示酒不足、饮酒失败；NPC 行动状态“坐诊或研读医学著作”过长，改为“坐诊”。
范围：排查并修复 NPC 面板与 GM 正式行动页的目标 NPC 错位，覆盖“前端赠酒 → 打开 GM → 指定喝酒 → 个人酒扣减 → `wine_consumed` 事件”真实链路；行动状态名称缩短为“坐诊”，保留无病人时研读医学著作的原玩法语义。不得改动赠酒 / 饮酒权威库存规则、情绪数值或其他 GM 行动结算。
完成（2026-09-07）：确认赠酒 / 个人库存 / ActionSystem 扣减均正常，实际错位发生在世界当前 NPC 与 GM 正式行动页默认 NPC 之间。GM 每次从关闭状态打开时，三个页签选择器统一对齐世界当前选中 NPC；面板打开后仍保留原有的逐页独立改选和本页路由。`work_clinic_doctor.name` 缩为“坐诊”，新增 description 保留空闲研读语义。T0063/T0384 端到端专项、完整 GM、NPC 面板、动态 station context、Schema、计划 Prompt 与 Main headless 均通过；Godot MCP 正式 Main 运行态复验通过，无 LLM 调用。版本 `0.0.681-t0384-gm-wine-target-sync`。

### T0383 驿站路边火炬与绿地树桩

状态：Done
用户授权（2026-09-07）：正式驿站路边关键处增加少量立杆火炬，与城门灯同步夜间点亮；绿地少量树桩，直径匹配站外树干，局部减草。
范围：纯表现节点和数据布点，不改建筑、工位、道路、碰撞、导航或外部植被密度。验收昼夜同步、稀疏布置、脚边减草及正式场景实机画面。
完成（2026-09-07）：6 处 2.5m 木杆铁篮火炬、4 个直径 0.62–0.76m 树桩正式接入；年轮切面 / 短根和低多边形火舌，火焰随游戏暂停。BuildingFunctionalLightController 直接同步夜间状态，18:00 开 / 06:00 关，无第二时钟。装饰旁去除 14 簇草，余 900 簇草与原 8 处花；站外 23812 簇未改。CPU 保留草实例源数据，避免 headless 渲染后端读取 MultiMesh 变换为零影响清理；保留实例位置 / 色彩 / 随机序列不变。所有布点通过道路边缘及最大建筑地块避让；纯表现，不新增碰撞或导航。
验收：T0383 正式 D3D12 四张昼夜远近景和边界专项通过，修正铁篮自投影造成的放射状阴影；T0135-P8A 原建筑功能灯回归及 T0129C-A3a 正式导航回归通过（239 碰撞源 / 11 门），最终日志无错误。截图 / 日志 artifacts/visual_qa/t0383/。F5 正式游戏可见，夜间自动点亮，无需新 GM；版本 0.0.680-t0383-station-ground-decor。

### T0379-I 已认可陨石正式接入

状态：Done
收尾修复：T0238 日志发现清理已释放陨石时，AbilityAudioController 在有效性检查前强转播放器导致 `Trying to cast a freed object`。已在转换前检查引用，不改变声音资产 / 增益 / 暂停合同；针对性重跑弹坑生命周期与 P10H 音频并检查完整日志。
用户授权（2026-09-07）：可以，接入游戏。
范围：正式 PietySystem 使用已认可 R1 岩体 / 尾焰 / 冲击与弹坑，原选点 HUD / CameraRig 强震、虔诚 / 伤害 / 无友伤 / 建筑禁投 / 友军排出 / 碰撞 / 战后清理和 24 游戏小时淡化保持。共享已认可美术与独立 A/B；补齐正式暂停 / 时间接线，不接入试片 8＋4＋10 秒演示清理。验证权威、生命周期、音频、真实 GPU 正式场景和试片共享。

完成记录（2026-09-07）：已认可实现迁入 FormalMeteorArt，MeteorArtCandidate 保留兼容包装，正式与 B 共用 meteor_art.json 和原三种程序材质；A 保留 MeteorPresentation 原版。PietySystem 切换美术构造并调用共享纯表现轨迹采样，2.8 秒权威下落不变，末段加速与连续落地和试片一致。新增粒子统一跟随正式时间 / 暂停，下落粒子使用稳定现实秒，落地冲击 / 冷却使用原战斗表现秒。原燃烧区仅隐藏旧红盘，伤害、火焰和灯光保持；HUD 选点盘、CameraRig 原强震与两段音频未改。战斗超过试片 8 秒时岩体仍在，战后仅清岩体，24 小时弹坑由 PietySystem 独立淡化。

验证：T0165 电影化生命周期（只更新美术节点断言）、T0238 24 小时弹坑、T0181 建筑禁投 / 友军排出、虔诚陨石伤害 / 无友伤、P10H 音频、T0379 独立交互回归均通过；新增正式专项实际 D3D12 八图 / 断言 T0379_FORMAL_PASS，验证共享构造、消费虔诚、接触连续、暂停所有粒子 / 冲击 / 冷却、战后清理、12 小时 Shader 与坑缘 Alpha=0.5、24 小时最终释放。结果 artifacts/visual_qa/t0379_integration/，无运行错误；无 LLM、无需新 GM，未做战斗性能压力测试。版本 0.0.675-t0379-meteor-integration。

### T0382 虔诚蓄满短圣咏重新选材

状态：Done
用户反馈（2026-09-07）：T0380 临时复用的弥撒现场圣咏过于嘈杂，正式蓄满提示必须先提供候选试听，由用户确认后才能重新接入。

验收标准：立即停用被否决的正式提示音但保留弹窗；从干净、短促、神圣、格里高利风格且低现场噪声的来源制作独立试听包，所有候选仅在 `art_source/audio/reviews/`，用户确认前不进入正式 manifest / runtime，也不修改弹窗播放映射；确认后再制作正式 48 kHz 短音、许可台账和程序回归。

当前进度（2026-09-07）：原 T0380 圣咏已标记 rejected、从正式 manifest / runtime 撤下并关闭映射，弹窗暂时静音；`sample_pack_v24_piety_ready_reselect` 已生成 10 条约 3.5 秒、统一清理和淡出的候选，等待用户选定编号后再正式接入。

完成记录（2026-09-07）：用户选择 05。正式素材取同一 Stabat Mater 原录音 `00:13–00:18`，由试听版 3.5 秒自然延长到 5 秒，最后 1.5 秒渐弱至静音；不是循环或机械拉伸。生成 48 kHz 单声道 WAV，重新启用全局 World / SFX 映射；其余 9 条标记 rejected。正式资产恢复为 100 条，专项与资产 QA 通过。版本 `0.0.674-t0382-piety-ready-chant-05`。

### T0381 陨石下落与冲击音频的大招可闻性

状态：Done
用户反馈（2026-09-07）：陨石下落与砸地声音过小，镜头拉远后的衰减也不符合夸张大招事件。

验收标准：两段声音仍跟随实际陨石 / 落点保留方向感，但使用独立的大招空间档而不是普通 local；在常用俯视和拉远镜头高度保持清晰，关闭远距低通，同时分别提高下落与冲击基础增益；不改素材时长、暂停、虔诚、伤害或陨石时序；专项与相关回归通过。

完成记录（2026-09-07）：AudioManager 新增 `ability_priority` 3D 档，参考距离 120m、最远距离至少 1500m、清除远距低通；AbilityAudioController 只为陨石下落 / 冲击使用该档，并分别增加 +2 / +3 dB。两段仍挂实际陨石与落点、完整播放并跟随暂停。P10H、P10A、T0380 与 99 条资产 QA 通过；无玩法或 LLM 变化。版本 `0.0.673-t0381-meteor-audio-presence`。

### T0380 陨石虔诚蓄满弹窗与短圣咏提示

状态：Done
用户要求（2026-09-07）：大招陨石进度条真实蓄满时弹窗提示“驿站成员的虔诚感动了上苍，现在你可以请求一次天火摧毁敌军！”，并同步播放一次短促、带格里高利神圣诵唱感的声音；点击“太好了”关闭。

验收标准：仅在虔诚从未满真实跨到可施放上限时提示一次，持续满值、UI 刷新和重复无效加值不重播；施放消耗后再次蓄满可再次提示；弹窗只读 PietySystem 的提交后事实，不改变虔诚、陨石资格或施放；短音复用已确认弥撒圣咏制作成独立非定位 World / SFX 提示，受“音效”音量控制而不混入普通点击；与既有里程碑弹窗协调，不覆盖建筑完工、幼马命名或守波提示；专项和相关回归通过。

完成记录（2026-09-07）：PietySystem 仅在共享虔诚由未满真实跨到 100 时广播提交后 `piety_ready`；持续满值和无效加值静音，施放归零后再次充满可再次触发。MilestoneAlertPresenter 将提示纳入既有 FIFO，真正显示弹窗时才播放 4 秒全局 World / SFX 圣咏，文案与“太好了”按钮按用户要求落地。短音复用已确认弥撒圣咏前 4 秒并烘焙 50 ms 淡入、500 ms 淡出；正式资产由 99 增至 100 条。专项覆盖跨越一次、排队不抢窗、关闭、重复静音和二次充能重播；虔诚陨石、里程碑、音频基础、陨石音频及 Main headless 回归通过。既有“填满虔诚”GM 入口可直接验证，不新增权威入口；无 LLM。版本 `0.0.672-t0380-piety-ready-alert`。

用户修订（2026-09-07）：该临时圣咏被评价为过于嘈杂，声音部分由 T0382 重新选材；当前保留已认可的弹窗与触发边沿，但正式提示音已撤下并保持静音，资产总数回到 99。

### T0379 陨石完整生命周期独立美术试片

状态：Done（用户已认可 R1，正式接入见 T0379-I；下方保留试片历史）
2026-09-07 R1 已完成，仍待试片评价：保留用户认可的岩体，选点恢复正式 HUD 的 CylinderMesh 橙红半透明范围盘（同半径、0.035 高、64 段、Y=0.055、颜色与 emission），无效区域按正式方式隐藏。尾焰尺寸 ×2、速度 ×1.6，增加暖黄内焰与粒子密度；冲击尘浪扩张 ×1.2、略增体积与透明度，空气波加粗 / 加强并延续 1 秒，触地闪光略扩大，飞石 27→36。试片 A/B 均直接复用正式 CameraRig 的震动函数与 piety_ability.json：下落 2.8 秒 / 振幅 0.1 / 频率 9，落地 2 秒 / 振幅 0.82 / 频率 23，完整三轴波形和衰减不再弱化。headless 与 D3D12 T0379_PASS，新增范围盘一致性、震屏来源 / 参数 / 暂停与归位断言；26 张截图和完整生命周期回归通过，日志无运行错误。正式源和产品版本未改。
用户要求（2026-09-07）：平行于正式游戏制作优化陨石试片；可点击已充满的大招按钮选区域，观察下落、撞击、弹坑、岩体消失及弹坑持续后渐隐。

范围：独立场景默认新版 B，保留正式 MeteorPresentation 为原版 A；复用正式虔诚按钮和半径 / 2.8 秒下落配置，独立演示调度不消费正式虔诚或提交伤害。新版采用不规则低模岩体、局部熔岩裂缝、定向尾焰、分阶段冲击与不规则焦土坑缘。演示缩短战后岩体清理与 24 小时弹坑生命周期，明确为试片时间；可重复施放、取消选点、暂停、切换 A/B 和昼夜。正式模型、权威系统、音频、碰撞规则和启动入口不改。GPU 检查完整交互 / 生命周期、多视角与隔离；完成后记 Partial 待用户视觉评价。

完成记录（2026-09-07）：新增 MeteorArtTrial 独立入口，默认 B；实际点击正式十字架控件进入地面选点，左键施放、右键 / Esc 取消，按钮施放后耗空。原版 A 直接实例化正式 MeteorPresentation，候选 B 为独立子类，采用 240 面不规则岩体、程序熔岩裂缝、沿轨迹尾焰 / 余烬 / 烟、连续落地姿态、短亮闪 / 空气波 / 尘浪 / 飞石、余火冷却，以及起伏焦土与疏密不均的 19 块坑缘碎片。两版均 2.8 秒下落，落地停留 8 秒清理岩体，弹坑再保留 4 秒、10 秒淡出；结束后自动蓄满。演示时间不写回正式配置。1 / 2 切 A/B 并重置，重播落点保留上次目标，三视角 / 滚轮 / 昼夜 / 空格暂停 / H 隐藏工具栏。复用正式地表，不实例化 Main / PietySystem / LLM，无伤害提交。

验收：headless 和最终实际 D3D12 为 T0379_PASS，真实 GUI 鼠标按钮 → 地面选点、取消保留充能、越界拒绝、A/B 实例来源、2.8 秒落地、暂停时钟与粒子、岩体先清理 / 弹坑继续存在、透明度递减、最终释放与重新充能通过。A/B 各 11 张下落 / 冲击 / 岩体 / 昼夜 / 冷却 / 弹坑 / 淡化 / 清理图及 922×518 小窗和净图共 24 图；GPU 日志无运行错误。工具 tools/verify_t0379_meteor_art_trial.gd，结果 artifacts/visual_qa/t0379/。曾发现顶点颜色线性空间使焦土过亮，已显式转换并在最终 GPU 图核对。无需新 GM、无正式接入、无战斗性能压力验收；不为独立试片修改产品版本。

### T0378 昼夜背景音乐多曲轮播与曲尾淡出

状态：Done
用户要求（2026-09-06）：将 `sample_pack_v22_bgm_day_night` 中白天 01、03、04、05、07、12 与夜晚 05、06、10 正式加入游戏；按昼夜分别轮流播放，每首在结尾处逐渐减弱到完全静音后衔接下一首。

验收标准：白天 6 首与夜晚 3 首按各自编号顺序循环轮播；昼夜边界切换到对应播放池，敌人在场仍优先战斗音乐且战斗结束回到当前昼夜池；九首均为非单曲硬循环，在自然结束前淡出至静音，并由下一首淡入；音乐继续受“音乐”音量控制，不改变环境音、战斗或时间权威；资产清单、许可台账、专项与 Main / MainMenu headless 回归通过。

完成记录（2026-09-07）：正式生成白天 6 首（总长约 14:04）与夜晚 3 首（总长约 9:08）的 48 kHz 立体声 OGG 和 24-bit WAV 母版，统一到 -16 LUFS，并在每个文件首尾烘焙 2 秒淡化。AudioManager 支持保持旧调用兼容的非循环音乐和自然完成信号；WorldAudioController 以配置顺序轮播两个池，昼夜切换、战斗接管及清场恢复均不改时间 / 战斗权威。正式清单、QA、署名和总试听列表由 90 扩为 99 条；专项实际验证 `day 01→03`、`night 05→06`、战后继续 `night 10`，P10A、前端、Main headless 与 99 条文件 QA 全通过。功能可在正式主场景直接听到，无需新 GM、无 LLM；Godot MCP 端口被既有编辑器占用，以 recovery import 和 headless 完成验证。版本 `0.0.671-t0378-day-night-bgm-playlists`。

### T0377 英雄升级与技能点分配声音

状态：Done
用户要求（2026-09-06）：英雄升级和点击成功分配技能点时增加声音。

验收标准：只有等级真实提升时播放一次升级成功提示；只有技能点真实扣除且属性真实增加时播放一次分配反馈，失败、无点数或单纯刷新不发成功声；声音不随镜头衰减并受“点击音效”滑杆控制；复用已确认正式音频，不改变成长数值、经验阈值或技能点权威。

完成记录（2026-09-06）：EventBus 新增 `npc_level_up` 与 `npc_attribute_point_assigned` 两个提交后表现信号；NPCSystem 只在总经验真实跨过阈值获得技能点、或属性点真实消费成功后发出。InteractionAudioController 将两者映射为已确认 `sfx_ui_interaction_success_resource_collect`，通过 UI 总线全局 2D 播放；人物面板 `+1` 成功时高优先级成功语义替代同帧普通木质点击，只有一声，失败分配无声。P10G 覆盖升级、按钮分配、去重和失败静音，T0904 成长权威、P10A 音频基础回归通过。功能在正式 NPC 面板直接可见，无需新 GM、无 LLM；Godot MCP 编辑器连接仍未恢复，本轮以 headless 完成验证。版本 `0.0.670-t0377-progression-audio`。

### T0376 警报与核心战斗音效可闻性修复

状态：Partial（实现与自动化完成；Godot MCP 实机复验因编辑器连接中断待补）
用户要求（2026-09-06）：排查警报声，以及攻击、受击、击中、惨叫等整套战斗声音缺失、过小或随镜头拉远衰减过快的问题并修复。

验收标准：警报、敌人入场 / 战斗开始、单波清除、最终胜利与失败作为无位置的全局战局提示稳定可闻；攻击挥动 / 释放、命中、我方 / 敌方受击语气、昏迷倒地等既定核心战斗资产仍由权威事件驱动，在常用战术镜头高度不会因 3D 距离迅速消失，同时保留声源方位；不得靠修改用户 SFX 设置掩盖问题；自动化覆盖事件映射、空间参数和真实警报链，并完成 Godot MCP 实机验证。

完成记录（2026-09-06）：本机设置与运行时读回证明 Master / SFX / Combat 均为 0 dB、未静音，警报事件和 28 条战斗资产映射均存在；根因是普通 `AudioStreamPlayer3D` 的 `unit_size=12m` 会把 22m 高相机及水平距离共同计入逆距离衰减，并叠加默认 `attenuation_filter_db=-24dB` 远距低通。按用户修订，警报、入场、清波、最终胜利 / 失败改为 Combat 总线全局 2D；其余实战声使用 `combat_priority` 空间档（`unit_size=48m`、最远距离至少 480m、关闭远距低通），保持 3D 方位。P10E（28 资产、权威警报、全局提示、核心战斗空间参数）、P10A、combat_flow 与项目加载通过；未改用户音量、战斗权威、概率规则或资产。Godot MCP 修改前诊断读回警报确实播放于 51.66m、旧参数 12m / 280m / -24dB；修改后准备重启时编辑器连接 `ECONNREFUSED 127.0.0.1:8765`，故运行时试听复验待编辑器恢复。无需新 GM，未调用 LLM。版本 `0.0.669-t0376-combat-audio-presence`。

### T0375 局部音效可闻性、警报与界面声音补全

状态：Done
用户要求（2026-09-06）：补回敌袭警报声；放缓行动脚步、工作及其他局部 3D 音效随镜头拉远的衰减；NPC 面板中的回复情绪语气改为不受距离影响的全局播放；主菜单补齐木质 UI 点击声。
- 在统一音频入口调整全部局部 3D 播放器的听距和衰减尺度，保留空间方向感但避免镜头稍拉高即静音。
- 警报沿现有 `trigger_combat_alarm` 权威流程触发既定号角资产，固定从驿站内警报位置发声；不新增战斗结算。
- NPC 回复情绪语气改走 Voice 总线的 2D 播放；主菜单及其设置 / 存档 / 确认子界面的按钮复用既定木质点击与 toggle 声。
- 更新自动化回归并进行 Godot 运行验证；无需 LLM 或新 GM 入口。

完成记录（2026-09-06）：AudioManager 对所有 3D 单次声和循环声统一应用 `unit_size=12m`、清单基础 `max_distance ×4`，跑步由 18m 扩至 72m、工作由 28m 扩至 112m，仍保留逆距离方位衰减。CombatSystem 在统一警报成功提交后发送 `combat_alarm` 表现事件，CombatAudioController 从主厅入口位置播放既定号角。DialogueVoiceAudioController 移除 NPC 跟随声源，19 条男女九情绪回复全部改为 Voice 总线全局 2D；MainMenu 递归接入当前和动态创建的 BaseButton，普通按钮木质声、CheckBox / CheckButton 用 toggle。P10A（含 4× / 12m 实例参数）、P10C、P10D、P10E（28 资产与真实警报链）、P10F（global_2d）、P10G、T0354（真实设置按钮点击）均通过；Godot MCP 实机读回主菜单 43 个按钮接入、警报 3D 与语气 2D。未改音量分组、NPC / 战斗权威，无需新 GM，未调用 LLM。版本 `0.0.668-t0375-audio-audibility`。

### T0374 正式主厅山墙与窗洞完整性

状态：Done
用户授权（2026-09-06）：UI 暂缓；正式主厅也按主菜单已认可方式修复窗内空壳、屋顶下空洞。
- 共用已认可山墙 / 窗面木框 / 入口门表现；正式建筑维持不可进入、不揭顶的现有规则。只改美术，不修改碰撞、导航、工位、塔防部署、HP / 升级机制。
- 新增收口随六级门楼变化和摧毁废墟显隐，旧菜单 A 与正式菜单 B 外观保持。验证正式主厅多角度、六级、受损 / 摧毁及现有主厅 / 导航回归，无需 LLM 或新 GM。

完成记录（2026-09-06）：将已认可封面构造提取为 MainHallShellFinish，正式主厅默认安装于 BaseVisuals/Exterior，补四片实体山墙、木梁、49 个内凹不透明窗面 / 木框及双扇木门。六级隐藏被新门楼替代的两片小山墙，收口随原建筑摧毁 / 修复显隐；菜单包装入口和原版 A 隔离保留。原 229 个美术网格的层级、Transform 与 AABB 比对通过，新增 376 个纯表现网格，无碰撞 / 导航。实际 D3D12 正侧背 / 山墙 A/B、六级前后、受损 / 废墟共 24 图通过；T0352 外墙屋顶、T0132-P1 六级平台 / 部署 / 受损、T0129c-A3a 生产导航（239 源 / 11 门）及 T0371 菜单回归通过。图 / 断言 / 日志在 artifacts/visual_qa/t0374/。无需新 GM，未调用 LLM，未做战斗性能压力测试。版本 0.0.667-t0374-main-hall-finish。

### T0373 主游戏 HUD 独立 A/B 对比样片

状态：Partial（样片完成，待用户视觉评价）
用户授权（2026-09-06）：制作主游戏 HUD 对比样片，先看效果。
- 2026-09-06：用户要求 UI 先不管，样片保留并暂缓，未正式接入。
- 用户修订：撤销四周分散布局；尽量保留 HUD 左上、NPC 右侧、对话原区域，只调整区域内部排布和美术。参考策略游戏的稳定信息分区，并在样片中检查 NPC / 建筑 / 对话共存。
- 同一正式驿站、镜头和静态状态下，A 显示原 HUD，B 在原左上区域内部重排时间、资源、敌袭与防守操作，降低边框亮度和占屏面积；保留正式图标和数据来源。
- 新增独立入口，默认 B，1 / 2 切换，H 隐藏对比工具栏。静止调试启动、不调用 LLM；样片按钮不提交玩法操作。正式 Main / MainMenu / HUD 保持，未认可前不接入。
- 验证实际 GPU 新旧同镜头、多窗口比例、信息一致及启动隔离。可直接在独立场景查看，无需 GM；完成记 Partial 待用户评价。

完成记录（2026-09-06，按用户两次反馈修订）：HUDArtTrial 继承正式 Main，在子节点启动前设静止调试，固定第 1 天 12:00 与全站镜头。A 为原 HUD；B 保留左上 (12,8) 锚点，区域内部按时间 / 资源 / 敌袭与防守操作三层排布，暖黑底、暗细框、暖白文字；去掉重复标题和后端行。常态为 564×160，比原框面积减少约 28.4%。小窗口打开原 NPC 右侧面板时，HUD 依据其真实左边界在左上收窄换行，人物近景和 NPC / 建筑位置保持。增加“仅 HUD / NPC / 建筑 / 对话”上下文切换（下拉或 3），A/B 用 1 / 2，H 隐藏工具栏。B 的世界提示 → HUD → 详情 → 对话层级保证前后顺序；对话仍居中，仅在极小窗口压缩高度以保住底部操作。候选只读原数值 / 图标，按钮不执行玩法；NPC / 建筑信息可滚动，对话仅打开空草稿，不提交文本或调用 LLM。

验证：四档 922×518、1280×720、1600×1200、2560×1080 基础 A/B 八图，两档 NPC / 建筑 / 对话 A/B 十二图及 B 净图，共 21 图；静止启动、资源 / 倒计时一致、同镜头、HUD 不覆盖 NPC / 人物近景 / 建筑、A/B 不移动详情窗，以及候选小窗口对话边界 / 层级断言通过。截图与快照 `artifacts/visual_qa/t0373/`。正式 HUD / Main / MainMenu、人物和建筑源未改，版本保持 0.0.666；无需 GM。仍为静态样片，正式动态绑定、战时长文案、装备 / 库存 / 所有弹窗组合尚未验收；待用户评价后另行接入。

### T0372 主菜单主厅左侧弩床塔防提示

状态：Done
用户要求（2026-09-06）：在主厅靠近 UI 一侧增加一级台子＋弩床，沿用主场景样式暗示塔防玩法。
- 复用正式主厅一级防御平台与正式弩床美术，在封面左前屋顶位置组合展示；检查武器朝向、台面承托、菜单避让与多比例可读性。
- 仅接入已认可封面表现，不创建部署 / 战斗权威，不改人物、工位、主厅结构或旧版 A。直接可见，无需 GM，无 LLM。
- 用户修订：首版太靠中间，平台与弩床整体移回正式一级槽位的屋顶左前角，不为小窗口避让菜单而向中央挪动。

完成记录（2026-09-06）：MenuCoverCandidate 从正式 fixture / device 配置复用 main_hall_slot_03_platform 与 wall_ballista 纯美术场景，一级平台位于主厅局部 (-7, 3.05, 5)，弩床安装高 3.93、朝向前方，封面偏移归零，增加克制局部补光。正式模型、部署数据、人物、工位、镜头与旧 A 未改。实际 D3D12 五档窗口与正侧近景共七图、正式角落坐标 / 26＋62 模型部件 / 0.88 安装高差 / 无新增碰撞导航 / 无 Main / 无发射断言通过，日志无错误。720p、1080p、4:3、21:9 弩床无遮挡；922×518 极小窗口会被原菜单遮住大部，遵照用户角落优先要求保留原位置。截图与快照在 `artifacts/visual_qa/t0372/`，一次性验证 `artifacts/verify_t0372.gd`。版本 `0.0.666-t0372-menu-ballista`；可直接在正式菜单验证，无需 GM，未调用 LLM。

### T0370 主菜单封面第二版独立对比

状态：Done（用户认可新版并授权 T0370-I 接入）
用户授权（2026-09-06）：保留当前封面，按新的美术建议另做一版对比。
- 原 MainMenu / MenuCoverPreview 与正式启动入口保持；新增可运行 A/B 对比场景。
- 固定镜头、人物模型与站位、建筑和工位关系；候选精修庭院主体光、外围分层雾、深色踩实土地面，并在原 27 个树位使用已认可低模树。
- 实际 GPU 检查两版、不同窗口比例和微动；验证原版资源未改、无 Main / 玩法权威 / LLM。纯视觉独立入口可直接验证，无需 GM。
- 完成后保持 Partial（待用户评价），未经认可不接入正式主菜单。

### T0370-I 新版封面正式接入与 T0371 封面主厅完整性

状态：Done
用户授权（2026-09-06）：新版更好，切到新版；修复红框所示两处三角屋顶下空洞、窗内空壳与异常柱子。
- 正式 MainMenu 使用已认可新封面与平滑边缘雾，独立 A/B 保留旧版。
- 主厅修整仅应用于封面实例，补齐屋顶山墙与窗洞深度 / 内侧遮挡，保留人物、镜头、工位及游戏内正式主厅和玩法。
- 验证正式菜单、多比例、主厅正侧面与近景、旧版隔离及菜单交互，回写文档；直接在菜单可见，无需 GM，不调用 LLM。

2026-09-06：用户认可 T0370 新版，T0370-I / T0371 已完成。正式 MainMenu 改为已认可 MenuCoverCandidate，MenuEdgeFog 与对比样片共用新版平滑边缘雾材质工厂；原 MenuCoverPreview、旧雾 Shader 和 A/B 原版入口保留。新增 MenuMainHallFinish，仅在封面主厅实例上补齐两组屋顶的前后四片实体三角山墙、木梁收边、49 个窗洞的内凹不透明窗面与木框，以及入口双扇木门 / 铁条。窗洞尺寸按源网格的 X=-0.60…0.60、Y=1.05…2.31，封闭面嵌入窗洞并位于原内部体块之前，避免透见空壳或杂乱支撑。山墙按实测屋顶包围盒置于瓦檐内侧。原游戏主厅脚本 / GLB、旧封面、人物、镜头、工位和玩法未改，无新增碰撞 / 导航。版本 `0.0.665-t0371-menu-cover-finish`。

验证：实际 D3D12 正式菜单 16:9 / 4:3 / 21:9、主厅正面 / 左右侧 / 山墙近景共七图通过；49 个窗口覆盖范围、四片山墙、旧封面 / Environment 隔离、菜单无试验 UI / 无 Main，以及设置 / 载入 / 退出确认入口通过。T0358 固定镜头 / 五档尺寸 / 雾、T0359 七组工位、T0360 人物朝向回归均通过。GPU 日志无运行错误；截图 / 快照在 `artifacts/visual_qa/t0371/`，一次性验证为 `artifacts/verify_t0371.gd`。没有进行游戏内主厅结构或战斗压力测试，无需 GM，未调用 LLM。

#### T0370 样片历史记录（已认可，正式接入见上）

2026-09-06：T0370 已制作独立主菜单 A/B 对比样片，状态 Partial（待用户视觉评价）。A 直接使用当前正式 MenuCoverPreview；B 继承同一构图，减少全局灰雾与橙红泛光，增加人物暖色局部光、接触遮蔽、四个外围椭球雾团，以及带踩实土 / 墙脚和工位接合的深色地面。27 个原树位保留，复用已认可 StylizedTreeMeshes 并按旧树高度适配轮廓。新版屏幕边缘雾使用启动时生成的 512×512 静态噪声纹理，平滑原画面可见的方块状过渡；外围消散范围与 12 秒循环保持。人物模型、镜头、站位、主厅 / 围墙、工位和职业动作不改；原版场景 / 脚本 / 雾 shader 与正式启动入口均未改，版本保持 0.0.664。

验收：D3D12 三种比例（16:9 / 4:3 / 21:9）A/B、无 UI 与微动共十张截图，逐项核对镜头、八名人物、七个工位、故事道具、建筑 / 围墙 / 马匹 Transform、27 个树位、循环还原和 A 环境资源隔离，全部通过；碰撞 / 导航均为零、无 Main。原 T0358 五档尺寸 / 固定镜头 / 边缘雾回归通过。截图与 GPU 日志在 `artifacts/visual_qa/t0370/`，一次性验证在 `artifacts/verify_t0370.gd`。4:3 沿用原 cover 裁切，马匹右侧仍可能被裁；本轮未做性能压力测试，无 LLM / GM。F6 运行 `scenes/art/MenuCoverArtTrial.tscn`，1 / 2 或左上 A/B 切换，H 隐藏 UI；正式主菜单待认可后另行接入。

用户授权：先登记系列并执行第一项；2026-09-05 用户认可 T0362，要求接入游戏后推进下一项。固定人物模型、正式场景布局、建筑结构、工位、碰撞、导航与全部玩法机制；不接回 T0135-P1R3 六张已回滚贴图。每项先以独立可对比样片评审，认可后另行登记正式接入。

### T0362 地表与道路过渡样片

状态：Done（用户已认可样片，授权 T0362-I 正式接入）
优先级：P1

- 在独立 `GroundArtTrial.tscn` 复用正式主厅、角色与道路坐标，展示主厅前地表；A/B 共用镜头、光照与上下文。
- 尝试程序化哑光草泥色层、自然路肩、柔和断续车辙与无贴图立体低模草簇；不生成或导入写实地表图片。
- 提供原版 / 试案、远近与昼夜观察入口和实际 D3D12 对比图；验证新表现无碰撞、导航、工位或玩法系统，正式 Main 与 MainMenu 不引用试案。
- 样片完成后记为 Partial（待用户视觉评价），正式接入不属于本项。

完成记录（2026-09-05）：独立预览读取正式 42 段道路和八人初始坐标，复用主厅 / 公告牌 / 人物模型；新增草泥混合 Shader、922 簇共用网格的实心低模草和原版 / 试案、整体 / 近景、昼夜切换。修复首轮 D3D12 正弦 hash 的格状接缝并收敛色板。八张同镜头 A/B 截图及快照位于本机忽略的 `artifacts/visual_qa/t0362/`；headless 加载、D3D12 抓图与无 Main / 新增碰撞 / 新增导航断言通过。预览只包含主厅前的建筑上下文，其他建筑不实例化；不代表正式全站或夜间功能灯整合验收。新地表无网格检查告警，既有角色源数据告警不在本轮修改范围。此项直接在独立美术预览可观察，不是隐藏玩法功能，无需 GM。

### T0362-I 已认可地表正式接入

状态：Done
用户授权（2026-09-05）：非常认可 T0362 样片，要求按此接入游戏后继续下一项。
- 正式 Main 使用已认可色板、草泥过渡、断续车辙和立体草；抽出共享表现构造，预览与正式共用同一外观基线。
- 材质作用于既有地表网格，不把样片平面直接盖到建筑室内或河道；不改变地形高度、布局、人物、建筑、工位、碰撞、导航或玩法数据。菜单已确认构图保持现状。
- 检查正式广场、室内揭示、站区 / 外围、昼夜和性能；校验所有锁定源文件未修改。保留原版 A / 已认可 B 的独立对比入口。

完成记录（2026-09-05）：`StylizedGroundArt` 共享已认可配置 / Shader / 922 簇草，正式环境仅替换既有站区与外围平台网格材质，42 段旧道路表面隐藏并保留嵌入碎石。站区旧草及样片范围内旧自然散布退出，外围其余自然散布保留；未新增覆盖地面的平面。已修正外围旧网格所需的双面材质兼容。集成断言、P1 地表、P7R4 室内光、T0129C-A3A 生产导航回归通过，D3D12 广场 / 铁匠铺 / 全站昼夜六图通过。RTX 4060 Ti、1280×720、静态 Main 全站视角、无垂直同步、120 帧平均 16.108 ms / P95 17.797 ms；不是前后性能增益或战斗压力结论。源场景、人物、建筑 / 工位、导航数据未改；无 LLM、无需 GM。截图和性能数据：`artifacts/visual_qa/t0362_integration/`。

### T0363 低模植被家族与散布试验

状态：Done（用户已认可五组稀疏点缀；样片完成，正式接入另行登记）
- 以 T0362 用户反馈为前置，试验有厚度的草、矮灌木、碎石家族，统一轮廓与色板，消除当前部分写实叶片的违和感；保留通路和建筑包络，仅修改表现。
- 在独立预览中比较已认可地表配草与新增的草 / 矮灌木 / 碎石组合；本轮不自动把未认可的植被试案接入正式世界。

完成记录（2026-09-05）：新增 `VegetationArtTrial.tscn`，继承地表预览，A 保持已认可地表与草簇；B 增加三种矮灌木、三种碎石网格，43 组 / 163 个实例，六个 MultiMesh。种子、色板、道路退让、地块避让与间距由独立配置约束。headless 加载与 D3D12 远近 / 昼夜 / A/B 八图通过，新增碰撞和导航均为零；正式环境未引用此试案。可 F6 直接观察，无需 GM，不调用 LLM。图与快照：`artifacts/visual_qa/t0363/`。

修订完成（2026-09-05）：按用户反馈，灌木组间距由 3.7m 增为 16m、最多 5 组，最终 20 个灌木 / 碎石实例。修订后 D3D12 八图通过，已重新展示，仍不自动接入正式场景。六个新网格的 MCP 检查未新增告警。

### T0362-R1 站内外草密度连续性

状态：Done
用户要求（2026-09-05）：站外草的整体密度接近站内，允许随地形变化，但不能显得草只长在驿站内。
- 保持已认可站内地表与草的尺度 / 色板，把同款草扩展至站外平地；按连续噪声形成疏密，并在河谷 / 岩山脚退让。
- 只读既有地形轮廓，不改高度、河道、碰撞、导航或布局。分区 MultiMesh 控制全图散布的渲染范围。
- 核对正式近景、城墙两侧、站外平地和全图，检查无浮空草 / 河中草；复测渲染成本。

完成记录（2026-09-05）：原 922 簇不变；消除样片矩形外无同款草的问题，平地新增 23,812 簇、171 个 48m MultiMesh 分区，原地形高度 / 河谷轮廓 / 山脚边界只读。噪声密度与山脚渐退共用世界局部坐标，河谷两侧退让 1.2m，保持道路 0.7m 净空。可散布面积估算站内 9.46 / 站外 8.10 簇每 100㎡，约 0.86 倍；估算使用原区随机采样和外围抖动网格，不包含旧自然散布。headless 集成与 D3D12 十二张正式昼夜图通过；实际 GPU Transform 逐实例检查边界、河谷和道路。相同静态全站视角 120 帧平均 15.481 ms / P95 17.019 ms，未重跑战斗压力。人物、场景布局、建筑工位、碰撞与导航源无变更；无需 GM、无 LLM。

### T0364 树木轮廓与林缘层次试验

状态：Done（2026-09-05 原版密度 / 树位版获认可，授权正式接入）
- 试验不同树冠轮廓、树龄、疏密与群落关系；保持地图结构、自然阻挡、敌军出生和可见路线，先评审局部林缘样片。
- 独立 ForestArtTrial 复用已认可地表 / 草 / 稀疏灌木与正式森林退让规则；A 为原圆锥树，B 为阔叶、浅皮、针叶三类树的幼 / 成树六种模型，提供同镜头远近 / 昼夜比较。
- 本项只生成和展示纯表现样片，不改正式森林源、地图布局、人物、建筑工位或玩法权威；核对网格法线、接地、路线净空和树群轮廓。完成样片后等待视觉评价。

最新修订完成（2026-09-05，用户已认可）：用户否决加密版本，以原版树数量和稀疏 / 稠密区域为准，仅升级树模型、允许必要局部微调。删除候选密度覆盖和配置，直接继承正式分区目标 / 区域退让 / 点位生成；预览从正式全图分区按中心截取局部，而非改变森林 bounds 后重采样，因此 A/B 均为 823 棵且逐点坐标一致。树种随机流与正式 Transform 随机流分离，保持原版尺度范围、非均匀比例、朝向及远林加成阈值。树模型保持已认可六种，本轮没有需要移动的树位。D3D12 四镜头昼夜十六图、数量 / 原树位一致及正式退让 / 隔离断言通过；正式世界尚未接入。

修订完成（2026-09-05，密度待评价）：用户认可树模型，但否决整体减树。保留树形，取消保留率筛减，恢复正式树尺度范围及远林尺度加成；离站 40–100m 平地逐渐增密，深林以约 2.7 倍分区目标叠加 ±10% 噪声，河岸 / 山地保留原稀疏因子，不叠加平地增密。样片延伸到 Z=384，止于现有平地边缘内；同区域 A=996 棵、B=1,903 棵（阔叶 830 / 浅皮 430 / 针叶 643），新增密林镜头，D3D12 四镜头昼夜 A/B 十六图与退让 / 隔离断言通过。六个最终网格再次巡检无新增发现。驿站周围稀疏、普通平地冠层衔接；为未来敌军从丛林出现提供视觉纵深，不修改出生、显隐、战斗或道路权威。

首轮记录（2026-09-05，密度已被上方修订替代）：新增 ForestArtTrial 场景 / 控制器、TrialForestArtView 和独立配置。局部样片 X=-72…94、Z=54…238，A=457 棵，B=347 棵；六种模型使用分叉树干、错落树冠与平面法线，幼树同时减少冠团 / 枝层而非只缩放成树。模型获认可，整体减树被否决。预览站外原平台共用已认可地表材质，headless 与首轮十二图通过。正式源未改，图与快照目录按最新修订覆盖：`artifacts/visual_qa/t0364/`。

### T0364-I 已认可树木正式接入

状态：Done（2026-09-05 已接入正式 Main）
用户授权（2026-09-05）：认可原版密度 / 树位版，要求正式接入游戏。
已解决的阻塞记录：`godot_project.get_info` 返回 `connect ECONNREFUSED 127.0.0.1:8765`。仅登记任务和认可状态，未修改正式森林程序；恢复编辑器 MCP 后继续，不需要重新征求接入许可。
- 抽出共享树形构造，正式森林与预览共用六种模型；保持原版全图分区、7,393 个树位、Transform 随机序列、密度、出生与道路净空。
- 仅替换树美术；不改地形、自然碰撞、人物、建筑、工位、敌军生成或玩法机制。已认可地表继续生效，菜单封面保持原方案。
- 在实际 Main 检查前后林、河岸 / 山脚、昼夜和全图，逐实例比对旧版数量 / 位置 / 尺度朝向；跑相关导航、森林 / 灯光回归与渲染成本验证。

完成记录（2026-09-05）：用户重启后 MCP 恢复，沿用已有授权完成正式接入。`FormalForestArtView → StylizedTreeMeshes` 共享已认可六种树形，`environment_art.json.approved_forest` 默认启用；保留原版分区、树位、尺寸 / 朝向随机流和近林阴影范围。实际 Main D3D12 逐实例比较 7,393 棵原树的完整 Transform 全部通过，前林 3,086 / 后林 3,355 / 侧林 952，原道路 / 出生 / 山地 / 河岸退让不变。六种实例为阔叶成 / 幼 2,419 / 903，浅皮成 / 幼 1,168 / 456，针叶成 / 幼 1,750 / 697；新增碰撞 / 导航 / 交互为零。P4 森林、T0129C-A3a 生产导航、P7R4 室内光回归通过，保留 12 段自然树林阻挡；共享构造后的局部 A/B 仍各 823 棵，十六图及隔离 / 原树位断言通过。正式站区 / 城门 / 前后林 / 河岸 / 山脚 / 全图昼夜十四图与快照在 `artifacts/visual_qa/t0364_integration/`。RTX 4060 Ti、1280×720、关闭垂直同步，同一静态 Main 镜头各 120 帧：全站旧 / 新平均 14.338 / 13.774 ms，前林 5.573 / 3.118 ms，全图 23.025 / 16.974 ms（draw calls 9,796 → 7,374）；这是静态渲染 A/B，不代表战斗压力结论。人物、正式场景结构、建筑 / 工位、权威碰撞 / 导航与玩法源未改；无需 GM，未调用 LLM。版本 `0.0.659-t0364-forest-integration`。

### T0365 山地与地形表面试验

状态：Done（用户已认可高岩壁版，授权正式接入）
- 最新授权：允许在独立样片抬高视觉山体、重塑坡面为断崖；原正式山体源、碰撞、导航不变。原“只改材质、视觉顶点不动”不再约束候选副本；A 保留原版，树木 XZ / 数量 / 尺度不变，候选中的接地高度随新岩面调整。
- 用岩层、坡面色阶、山脚过渡和局部低模石组改善大色块；不改正式地形高度、碰撞、可行走范围和站区布局。
- 独立 MountainArtTrial 共用已认可地表和树林，以原山体为 A，对既有山体网格施加克制岩层 / 苔土过渡并点缀少量贴坡石组作为 B；保持所有原顶点和树位，提供同镜头昼夜比较。完成后待用户评价，不自动正式接入。

2026-09-05：T0365 独立山地样片已实现，状态 Partial（修订后待用户评价），尚未正式接入。原山体与岩块共 60 个网格 / Transform 保持原样，三个山层共 3,600 个原顶点未动；仅预览材质覆盖和七组 / 21 个装饰岩体。首轮浅色缓坡被用户指出像灰土坡，已改为灰岩色板、折线岩层、竖向岩壁与三级岩台；主岩体突出原坡面约 4.5–7.5m，改变的是装饰轮廓，原地形高度与通行边界不变。岩台底边 147 个顶点按真实原三角面重心插值埋入 0.12m，山脚共用已认可草土色板并渐退。A/B 共用正式地表、全部 7,393 棵已认可树及昼夜光照。headless 加载、D3D12 四镜头昼夜十六图与 MCP 运行检查通过；原网格顶点 hash / Transform 一致，无碰撞、导航、Main 或 LLM。截图和快照：`artifacts/visual_qa/t0365/`。人物、建筑、工位、正式环境配置与生成器源未改，版本保持 0.0.659；直接在独立样片观察，无需 GM。本轮不是正式场景整合、导航回归或战斗性能验收。

2026-09-05：按用户“山再高一点、更有悬崖峭壁感觉”的最新要求，T0365 样片改为真正抬高候选视觉山体，状态 Partial（待评价），尚未正式接入。三个原山层生成独立候选 Mesh 副本，保持山脚和 XZ 足迹，抬升山脊并把主要爬升集中为陡直岩壁；近 / 中 / 远层最高 35.7 / 64.7 / 105.4m。旧版原网格 / Transform / 顶点 hash 仍保留，A 显示完整原山体，B 显示高岩壁副本及七组岩台；不再要求候选视觉顶点保持原高度。3,036 个山地树木 / 散布实例在 B 中按真实候选三角面调整接地 Y，A 恢复原 Transform；7,393 棵树的总数、XZ、尺度 / 朝向不变。旧岩块复用并在候选坡面重新接地，新增岩台底边仍埋入 0.12m。headless 加载、D3D12 四镜头昼夜十六图、原资源 hash、候选高度、实际 GPU A/B Transform、隔离断言通过。MCP 已打开修订样片；无新增碰撞 / 导航 / Main 或 LLM。正式地形、碰撞、通行、人物、建筑和工位均未改，版本不变。图与快照：`artifacts/visual_qa/t0365/cliff_revision/`。正式整合与高岩壁遮挡 / 碰撞一致性尚未验收。

### T0365-I 已认可高岩壁正式接入

状态：Done
- 用户授权：认可 T0365 高岩壁版并要求接入游戏。抽取样片与正式共享构造，保持 35.7 / 64.7 / 105.4m 山峰、岩层和七组岩台；树林 XZ / 数量 / 尺度不改，仅山上表现实例 Y 接地。
- 正式山脚、碰撞、导航、人物、建筑与工位权威保持原值；验证正式 Main 昼夜、原资源 / 实例合同和生产导航，检查高岩壁不侵入既有通行范围。无需新增 GM，不调用 LLM。

2026-09-05：用户认可高岩壁样片并授权接入，T0365 / T0365-I 均 Done。`FormalGroundSurfaceArtView → StylizedMountainArt` 按 approved_mountain 默认启用，正式与样片共用同一套网格、岩层材质、岩台及接地构造。正式三层最高 35.7 / 64.7 / 105.4m，七组 / 21 块岩台，3,036 个山地表现实例只调整接地 Y；全图 7,393 棵树的 XZ / 尺度 / 朝向与原版一致。旧 60 个山体 / 岩块 Mesh 与顶点 hash 保留用于对照，正式显示已认可高岩壁，原岩山碰撞高度与导航源不改。实际生产导航最东 X=59.5、山体最西 X=120，二者相距 60.5m；生产导航回归保留 239 个静态碰撞源（含 4 段岩山 / 12 段密林）、11 个建筑门连接。GPU 全树位回归、P4 森林、P7R4 室内光与正式 Main 六镜头昼夜十二图通过，共享构造后样片十六图和 A/B 隔离 / 接地断言通过。RTX 4060 Ti、1280×720、关闭垂直同步、同一静态 Main 镜头各 120 帧：全站旧 / 新平均 13.997 / 13.488ms，山体 5.714 / 4.030ms，全图 16.993 / 17.038ms（P95 18.200 / 18.018ms）；未重跑战斗压力，不能据此推断实战帧率。人物、正式场景结构、建筑 / 工位与玩法源未改；无需 GM，未调用 LLM。正式图 / 快照 / 性能数据：`artifacts/visual_qa/t0365_integration/`，共享样片：`artifacts/visual_qa/t0365/shared_revision/`。版本 0.0.660-t0365-mountain-integration。

### T0366 河流与河岸表现试验

状态：Done（用户已认可纵向流痕 / 水岸石组 / 白沫版，授权接入）
- 在既有河道边界内试验深浅水色、流向、少量水纹、接岸过渡和岸石；不改变河道位置、地形碰撞或导航。
- 独立 RiverArtTrial 对比原版 A / 候选 B，共用已认可地表、树木、高岩壁与昼夜光照。读取原河道轮廓构造贴合水流，不抬高水面或改河岸原顶点；完成后待视觉评价，正式接入另行执行。

2026-09-05：T0366 独立河流样片已实现，状态 Partial（待用户视觉评价），未正式接入。新增 RiverArtTrial，A 保留当前蓝色水带 / 分色河岸，B 使用偏灰绿深浅水色、顺河细水纹、断续贴岸亮纹、草岸到石滩 / 湿岸渐变及统一灰色岸石。水体副本只补充横河 UV、纵向流动坐标和宽度属性，120 个原顶点位置逐点相等，无水位 / 河道位移；52 个河岸 / 岸石网格的资源、顶点 hash 和 Transform 不变。岸石沿用现有位置与数量，未新增河边密集装饰。A/B 共用已认可地表、全部 7,393 棵树、高岩壁及昼夜表现。headless 加载、D3D12 四镜头昼夜十六图、8 / 10 秒水纹两帧及 MCP A/B 检查通过；无碰撞 / 导航 / Main / LLM。水纹支持暂停观察。图与快照：`artifacts/visual_qa/t0366/`。正式环境配置、源地形、人物、建筑工位与玩法未改，版本仍为 0.0.660；无需 GM。尚未进行正式场景整合或实战性能验收。

2026-09-05：按用户反馈修订 T0366：原先虽沿下游移动但横向排列的水纹，已改为沿河道延伸的纵向细流痕；新增七处 / 21 块大小错落的低模水岸石，部分伸入水面打破岸线。石组按原河道 profile 取样，中心位于水线两侧 0.4m 内，底部埋至水面下 0.4m，顶部露出；中央至少 40% 水道保持净空。Shader 以这 21 块石头的水线轮廓定位白色泡沫，迎水侧 / 两侧较明显，下游留短而断续的尾流，与纵向流痕共用动画时钟。泡沫只画在原水体上，无新增泡沫平面或碰撞。120 个原水体顶点和 52 个旧河岸 / 岸石 Mesh / Transform 仍不变，树木仍为 7,393 棵。headless、D3D12 四镜头昼夜十六图与两个流动相位、MCP A/B / 接地 / 水道净空断言通过。当前样片已打开近看 B，状态 Partial 待评价，未正式接入；正式河道、水位、碰撞导航、人物和建筑工位未改，无 LLM、无需 GM、版本不变。当前图与快照：`artifacts/visual_qa/t0366/shore_revision/`。

### T0366-I 已认可河流正式接入

状态：Done
- 用户授权：将纵向流痕、七处水岸石组与绕石白沫版正式接入。正式 / 样片共用构造，保留原水位、河道、120 个水体顶点和原河岸 / 岸石位置。
- 验证正式 Main 河岸、站区昼夜与动画、原地形 / 导航 / 灯光回归及静态渲染开销；不改人物、建筑、工位、碰撞与玩法。无 LLM，无需新增 GM。

2026-09-05：用户认可纵向流痕、水岸石组和白沫版并授权正式接入，T0366 / T0366-I 均 Done。`FormalGroundSurfaceArtView → StylizedRiverArt` 按 `approved_river` 默认启用，正式与独立 A/B 样片共用同一构造与材质。保留灰绿深浅水色、顺河细流痕、草岸 / 石滩 / 湿岸过渡及七处 / 21 块水岸石，白沫贴合迎水侧与绕石边缘。原水体 120 个顶点逐点相等，52 个旧河岸 / 岸石 Mesh、顶点 hash 和 Transform 保持；水位、河道及全图 7,393 棵树不变。新石组最东 X=-89.03，实际导航最西 X=-59.5，未占用可行走区域。河谷回归保留 8 段河岸碰撞，生产导航保留 239 个静态源与 11 个建筑门连接；室内灯回归通过。正式 Main headless / D3D12 几何与动画检查、六镜头昼夜十二图，以及共享样片四镜头昼夜十六图和两个流动相位通过。RTX 4060 Ti、1280×720、关闭垂直同步、静态 Main 各 120 帧：全站旧 / 新平均 13.277 / 13.015ms，河流 8.124 / 7.148ms，全图 16.720 / 16.193ms（P95 18.244 / 16.698ms）；本轮未重跑战斗压力，测量波动不等于确定性能增益。人物、场景布局、建筑 / 工位、碰撞导航与玩法源未改；无需 GM，未调用 LLM。正式图 / 快照 / 性能数据：`artifacts/visual_qa/t0366_integration/`，共享样片：`artifacts/visual_qa/t0366/shared_revision/`。版本 `0.0.661-t0366-river-integration`。

### T0366-R1 河流速度与自然岸线修订

状态：Done
- 用户反馈正式水流太慢、水陆边界笔直像人工水渠。加快可见流动，增加不对称浅滩 / 冲刷凹口与不规则水线；延续低模风格和克制石组。
- 在共享河流表现中修订并验证正式 Main / 预览，不移动原河道、水位、源岸网格或碰撞导航，不改人物、建筑、工位与玩法。允许纯视觉岸滩向水内局部伸入以打破旧直线；检查连续接合、水道净空与绕石泡沫。

2026-09-05：T0366-R1 已按用户“水流慢、边界像人工水渠”的反馈修订并更新正式 Main。flow_speed 由 0.65 提到 2.8（约 4.3 倍），缩短纵向流痕的连续长度并提高可见度，绕石白沫同步加快。增加两片纯表现岸滩，共 1,202 行采样：两岸独立多尺度噪声生成浅滩凸出，石组附近缩回形成冲刷凹口，水线向原水面内伸入约 0–2.20m，每侧不超过当地宽度的 25%，中央至少一半水面保持开放。岸滩外侧按原岸面实际三角形插值接地、内侧沉入水下；2048×1 RG 浮点查找纹理让浅水色和零星岸缘亮纹跟随新轮廓，避免保留旧直线色边。原水体 120 个顶点、52 个旧岸网格 / 岸石资源与 Transform、七处 / 21 块石头及 7,393 棵树均保留；原水位、河道、人物、布局、建筑工位、碰撞导航与玩法源未改。新岸滩最东 X=-84.95，实际导航最西 X=-59.5，无通行重叠。headless / D3D12 集成断言、正式六镜头昼夜十二图、预览十六张 A/B 图和两个流动相位通过；河谷 / 生产导航回归保留 8 段河岸碰撞、239 个静态源及 11 个门连接。静态 720p 原始河流 A / 当前 B 各 120 帧平均：全站 14.124 / 14.480ms、河流 8.570 / 7.948ms、全图 17.235 / 17.407ms；未复测战斗压力，不能把不同轮次的测量波动解释为性能变化。正式结果：`artifacts/visual_qa/t0366_r1_integration/`；预览：`artifacts/visual_qa/t0366/natural_shore_revision/`。无需 GM、未调用 LLM；版本 `0.0.662-t0366-natural-river`。

### T0368 建筑与围墙地表接合

状态：Done
- 用户要求建筑和围墙与地面更自然地融合。按实际建筑基座 / 墙段位置形成贴脚压实土、克制接触暗部与入口磨损，并与原草土 / 道路渐变衔接。
- 仅改地表表现，固定建筑模型结构、地板、工位、位置、围墙 / 门结构、碰撞导航与玩法；验证主厅、旋转建筑、围墙内外、城门和昼夜。无需 GM，无 LLM。

2026-09-06：T0368 建筑与围墙地表接合已完成并接入正式 Main。新增 BuildingGroundContactArt，在既有建筑生成完成后读取 12 座主要建筑、2 座附属小屋和 14 段围墙的真实基座 Mesh AABB / Transform，而非规划地块；将其边缘投影到原地面材质，形成贴脚暗土、压实土渐变与克制细粒积尘。12 处建筑入口按 building_spatial.entry_route 接续原道路；正 / 后门仅在门柱附近补充接地色，不跨门洞画硬边。站内原地面与站外原平台共用一张 1024×1024 RGBA8 静态遮罩，逐像素地表噪声打散土边；无需覆盖平面、外部贴图、额外模型或逐帧 CPU 更新。修订中撤掉了第一版过亮的浅色描边，最终土色沿用已认可道路色板并略压暗。新增网格 / 碰撞 / 导航为零，人物、建筑 / 围墙 / 门结构、基座资源与位置、地板、工位、道路和玩法源不改。headless / D3D12 几何保留与旋转基座遮罩对齐断言、主厅 / 铁匠铺 / 围墙内外 / 正门 / 菜园 / 全站七镜头昼夜 A/B 共 28 图通过；生产导航保留 239 个静态源和 11 个门连接，室内灯回归通过。RTX 4060 Ti、720p、关闭垂直同步、同一静态全站各 120 帧，A / B 平均 13.740 / 12.882ms，draw calls 均为 6,945；该测量不代表确定的性能增益，战斗压力未重测。图 / 快照 / 测量：`artifacts/visual_qa/t0368/earth_revision/`。直接在 Main 看建筑脚部和围墙两侧，无需 GM、未调用 LLM；版本 `0.0.663-t0368-building-ground-contact`。

### T0369 驿站内少量白黄野花

状态：Done
- 将驿站内少量现有草簇一对一换成同样大小的白色 / 黄色普通低模野花，零星点缀；保留站外草、原散布总量与其余草位置，避开道路和建筑。
- 直接在正式 Main 检查远近 / 昼夜，固定人物、建筑、工位和玩法，无新增碰撞导航，无需 GM、无 LLM。

2026-09-06：T0369 已按用户要求，将驿站内 8 簇现有草一对一替换为普通低模小野花，白色 / 黄色各 4 簇，每簇 3 朵、细茎和少量绿叶。使用独立种子从原草位选取，必须在围墙内、距道路边缘至少 1m、沿用原建筑地块退让，花簇相隔至少 9m。新花网格包围盒严格匹配原草簇，完整沿用原位置 / 尺寸 / 朝向；中央原散布区 922 个位置保持，表现为 914 簇草与 8 簇花，外围 23,812 簇草不改。两种花共用各自 MultiMesh，不增加散布总量，无碰撞 / 导航 / 工位或玩法改动。headless 加载、实际 Main D3D12 逐点比对原 / 新 Transform、原草颜色、花簇尺寸、数量、站内退让和间距通过；前后区域共四处花簇的近景 / 环境与全站昼夜截图已核对，白黄花均为小型点缀。图 / 快照：`artifacts/visual_qa/t0369/` 和 `artifacts/visual_qa/t0369/front_views/`；一次性验证脚本在忽略目录 `artifacts/verify_t0369.gd`。无需 GM、未调用 LLM，本次未复测战斗压力。版本 `0.0.664-t0369-station-wildflowers`。

### T0367 已认可样片的风格统一与整合评审

状态：Todo
- 汇总用户认可的试案，检查常用镜头 / 昼夜 / 角色可读性，并测量帧时间与渲染成本；提出最小正式接入清单。人物模型、建筑结构、工位与玩法保持不变，正式接入需用户认可具体样片。
...
## T0158R1 NPC 外貌辨识度与正式模型二次校正

状态：Done
优先级：P1
前置任务：T0158, T0130-P8R2
涉及文档：`CURRENT_STATE.md`, `TASKS.md`, `AI_NPC_SYSTEM.md`, `MODULE_INDEX.md`, `DEV_LOG.md`

任务目标：

- 纠正 T0158 仍残留的生产模型错配，并把 8 段流水账式“发色 + 胡须 + 短衫 + 皮带”改为可区分的人物轮廓。
- 统一使用“第一眼轮廓 / 发式或面部 + 主色服装 + 身份物件”的结构；动态工作道具必须注明出现条件，不把动作态写成常驻事实。
- 继续由 `data/npc_profiles.json.appearance` 同时服务人物背景弹窗与 `npc_setting`，不修改模型、人设决策、Prompt Schema 或调用频率。

验收结果（2026-09-07）：

- 托马以短粗体型、棕布头带和马刷区分；布鲁诺纠正为正式模型的秃顶、圆肩、暖红围裙轮廓；伊沃突出瘦长身形和长尾头巾；格伦突出宽肩方脸和右腰铁锤。
- 艾达突出高束赤褐发、细长眉眼与蓝灰轻甲；马塞尔补入 P6R 后的圆润灰白削发冠；莉娜突出脸侧垂发、冷青蓝长衣与药包；欧文补齐短壮工头轮廓、围巾、土色工装、护目镜和双侧工具袋。
- 马刷、铜勺、铁锄、铁锤和护目镜均写明工作 / 闲时切换；没有把动态道具误写成全天姿态。新增专项锁定 8 人各自识别锚点、生产场景存在、档案与共享 `npc_setting.appearance` 一致，以及布鲁诺旧“短发”不再回归。
- JSON、后端 Schema、Mock Model Adapter、Mock 对话 endpoint、T0158R1 专项和 Main headless 通过；Godot MCP 正式 Main 读取到 8 条更新后的运行时档案，编辑器无新增错误。完整旧档案专项仍只命中已登记的布鲁诺 `speech_style` 超历史 80 字上限债务，与本次外貌改写无关；当前文本 LLM provider 明确配置为 Mock，未执行真实文本模型调用。
...
- T0386（2026-09-08）已用新日历与供给侧覆盖上述历史时点：五波为 D1 23:00 / D3 08:00 / D4 04:00 / D5 12:00 / D6 18:00，阶段周期 `4800 / 4500s`，松散木 8 + 首塔已投入木 4；敌军和其他 T0121 战斗合同未改。固定第五波构筑回归仍通过，但新日历连续经营概率尚未由 T0122 证明。
...
任务目标：用真实玩家从开局连续推进到 T0386 的第 6 日 18:00 第五波，记录各波征募人数、构筑、速度档操作、战斗时长、NPC / 建筑战损、恢复人时、失败原因和工作流 / 虔诚流选择，校准 T0121 / T0386 的首次与学会后胜率 KPI。盲测只调整分布性参数，不回退已确认的波次日历、唯一失败条件、无套利和制造成本必须计入等结构规则。

自动连续回放进展（2026-08-11）：

- T0386（2026-09-08）已把工具时间轴改到第 6 日第五波，并改为按权威下一波 T-3h 备战、T-20min 鸣警；下列第 7 日结果仅是旧日历历史证据。本轮压缩日历完整回放运行耗时异常长，未取得新的全程结果，任务继续 `Partial`。
...
## T0061R1 NPC 根本文案与初始长期记忆母语化润色

状态：Done
优先级：P1
前置任务：T0061, T0158R1
涉及文档：`CURRENT_STATE.md`, `TASKS.md`, `AI_NPC_SYSTEM.md`, `MEMORY_AND_INFO_SPACE.md`, `PROMPTS.md`, `API_BUDGET.md`, `MODULE_INDEX.md`, `DEV_LOG.md`

任务目标：

- 保留 T0158R1 的 8 条 `appearance` 原文不动，润色 `npc_profiles.json` 中其余面向玩家 / 模型的人设文案，以及 `npc_initial_long_memory.json` 的 24 篇初始日记和全部知识图谱中文标签。
- 不改变角色经历、到站顺序、人物关系、职业立场、欲望、恐惧、底线、建筑常识或任何结构化技术键和值；只修正不自然语序、成分残缺、翻译腔、条目腔和程序说明腔。
- 让档案像熟人介绍，日记像人物回望自身经历，知识图谱像角色对人和地方逐渐形成的认识；整体自然、具体、娓娓道来，同时保持八人各自的观察角度与表达节奏。

验收标准：

- 8 条外貌逐字保持 T0158R1 版本；档案各字段类型、条目数、身份、能力和数值不变。
- 8 人各 3 篇初始日记的时间标签、事实锚点和到站顺序不变；“往昔·近日”仍不预知敌情。
- 初始知识图谱主体、关系键、技术 `value`、可信度、日期、时间和总记录数不变，只允许改中文 `subject_label / relation_label / value_label`。
- 本地文案、JSON、Schema、Mock、六类共享身份 / 长期记忆注入、首次反思、NPC 面板及 Main headless / Godot MCP 验收通过；若文本真实 provider 可用，再验证人物事实与语气没有漂移。

完成记录（2026-09-07）：

- 8 人除外貌外的根本人设文案与 24 篇初始日记已完成母语化润色；外貌专项确认 T0158R1 的 8 条 `appearance` 逐字不变。布鲁诺原 84 字 `speech_style` 同步收紧为自然完整的 70 字内表达，旧长度债务消除。
- 初始知识图谱已全量审读：原本自然的人物观察与职业旧事不为制造差异而改写；守备官四类认识和偏程序化的建筑知识改成角色口吻。修改前后剥离日记正文与三类中文显示标签后的技术骨架 SHA-256 同为 `03b3b65b70a1f7252d540cc02cd9c7e696e4a031f12b173aa987a65f15917a96`。
- `verify_npc_initial_long_memory.py` 不再把 8 篇近日故事锁死为整段快照，改为锁定人物、物件和事件事实锚点，并兼容自然中文对“未知 / 尚未经战时考验 / 以后看实际行为”的等义表达。
- Python 文案 / 结构专项、`verify_npc_character_profiles.gd`、T0158R1 外貌专项、初始长期记忆 Godot 专项及 Godot MCP 正式 `Main.tscn` 实跑通过，编辑器错误日志为空。文本 `LLM_PROVIDER=mock` 且未配置文本 Base URL / Model，故没有可执行的真实文本 provider 验收。
...
目标：作为参赛加分项加入语音和情绪输入，但不影响核心 Demo 可运行性。正式方案固定使用阿里云百炼华北 2（北京）非实时 `qwen3-asr-flash`；完整合同见 `docs/VOICE_INPUT_AND_EMOTION.md`。

---

## T1600 语音输入完整方案与 Provider 定版

状态：Done（仅文档设计；已授权从 T1601A 开始实施）
优先级：P1
前置任务：T0701, T1401A
涉及文档：`VOICE_INPUT_AND_EMOTION.md`, `TASKS.md`, `CURRENT_STATE.md`, `MODULE_INDEX.md`, `TECH_ARCHITECTURE.md`, `GODOT_ARCHITECTURE.md`, `DATA_SCHEMA.md`, `AI_NPC_SYSTEM.md`, `MEMORY_AND_INFO_SPACE.md`, `UI_UX.md`, `PROMPTS.md`, `API_BUDGET.md`, `DEV_LOG.md`, `game_design.md`

任务目标：

- 将用户要求的麦克风入口、录音覆盖层、手动 / 30 秒自动完成、整段识别、文字与情绪拼装、追加草稿、识别失败降级和发送字数校验整理为可直接实施的合同。
- 正式 Provider 固定为阿里云百炼北京地域的非实时 `qwen3-asr-flash`，Godot 只请求游戏后端，百炼 Key 只保存在服务器。
- 明确 Provider 原生七类情绪；第一版不伪造“低沉、急促、嘲讽”。
- 把实现拆成可逐步验收的 T1601A / T1601B / T1601C / T1603 / T1602 / T1604；本任务完成后等待用户确认，不修改程序。

验收结果（2026-09-07）：

- 新增 `docs/VOICE_INPUT_AND_EMOTION.md`，覆盖前后端合同、UI、状态机、音频接线、错误、隐私、费用、测试矩阵与分步实施顺序。
- 玩家语音最终只形成普通可编辑 `speaker_text`，不新增 NPC 对话 Prompt、结构化互动或权威结算分支。
- 用户于 2026-09-07 将发送上限确认为 300 字；文本框保持无限暂存，超限弹窗文案固定为“输入文字超过上限300字”。
- 本轮只改文档，没有调用真实 API，没有修改场景、代码、配置或 GM 面板。
...
优先级：P2
前置任务：T1600
涉及文档：`VOICE_INPUT_AND_EMOTION.md`, `UI_UX.md`, `GODOT_ARCHITECTURE.md`, `TECH_ARCHITECTURE.md`

任务目标：

完成 T1601A、T1601B、T1601C，使玩家能够录制最长 30 秒的语音，并通过显式 Mock 后端把识别结果追加到输入框；本父任务不要求真实百炼验收。

验收标准：

- 输入框右侧有麦克风图标按钮；录音中在对话中央显示闪烁麦克风、计时、“正在录音”和“完成”。
- 点击“完成”或录满 30 秒都只提交一次完整录音。
- 识别文字追加到现有草稿末尾，不覆盖、不自动发送。
- 语音识别失败时可手动输入。
- NPC-NPC 旁听不显示麦克风；会话关闭 / 取消 / 挂起 / 完成时迟到结果不得进入其他会话。
- 不影响现有文本对话、攻击、特殊 toggle、逃离挽留和会话生命周期。

完成记录（2026-09-07）：T1601A 后端 Schema / 显式 Mock、T1601B 本地录音 UI、T1601C multipart Mock 闭环与追加均完成。MCP 恢复后，正式 Main 运行态确认识别段正确追加、不自动发送、迟到结果忽略、Recorder 回到 idle，编辑器无错误。

---

## T1601A 语音配置、后端 Schema 与显式 Mock 接口

状态：Done
优先级：P2
前置任务：T1600
涉及文档：`VOICE_INPUT_AND_EMOTION.md`, `DATA_SCHEMA.md`, `TECH_ARCHITECTURE.md`, `API_BUDGET.md`, `backend/README.md`, `MODULE_INDEX.md`

任务目标：

- 新增 `data/dialogue_input_config.json`，统一保存 300 字、30 秒和 6 MiB 上限，不保存任何秘密凭据。
- 新增 `backend/schemas/voice_input.py`、`backend/services/voice_model_adapter.py` 和 `POST /voice/analyze`。
- 第一阶段只接显式 `VOICE_PROVIDER=mock`，覆盖 multipart WAV、请求 / 响应 Schema、七类情绪枚举、错误码和 usage 记录骨架。
- 服务端使用 WAV 文件头复核时长、大小和格式；失败不调用 Provider。

验收标准：

- 合法 WAV 得到稳定 Mock 转写与合法情绪；损坏、空、超过 30 秒或 6 MiB 的文件得到明确 4xx。
- 响应携带 request / dialogue id、transcript、emotion、emotion_label、duration 和模型来源字段。
- Mock 只在显式开发配置下工作；没有真实 Provider 时不伪装真实成功。
- API Key、Authorization、Base64 音频和原始 WAV 不进入日志。

完成记录（2026-09-07）：

- 新增统一配置、Voice Pydantic Schema、独立 `VoiceModelAdapter` 与 `POST /voice/analyze`；未配置时保持禁用，当前仅允许显式 `VOICE_PROVIDER=mock`，其他 Provider 返回 `voice_provider_unavailable` 且不回退。
- 服务端已复核 multipart 元数据、6 MiB、PCM WAV、空录音与 30 秒边界；Mock 输出固定合法转写与百炼原生情绪，未知情绪降级为 `none`。
- `/health` 暴露非敏感语音配置，`/debug/llm_usage.voice` 记录 request / dialogue / NPC、Provider、model、时长、成功 / 失败和 `fallback_used=false`，不保存音频正文。
- `verify_voice_input_mock_endpoint.py`、Schema 与既有 Model Adapter 回归通过；本子任务不调用真实百炼，真实 Provider 仍属于 T1602。Godot 尚无语音请求桥，因此本基础接口用 Python 自动化验收，Main 可见验收留在 T1601C / T1604，不新增临时 GM 入口。

---

## T1601B Godot 麦克风录制与对话覆盖层

状态：Done
优先级：P2
前置任务：T1601A
涉及文档：`VOICE_INPUT_AND_EMOTION.md`, `UI_UX.md`, `GODOT_ARCHITECTURE.md`, `MODULE_INDEX.md`

任务目标：

- 开启 Godot 音频输入，增加静音 `MicRecord` 总线、`AudioEffectRecord`、麦克风播放器和录音控制器。
- 在 `DialogInputEdit` 右侧增加本地 SVG 麦克风按钮。
- 增加 `IDLE / RECORDING / ANALYZING` 状态机、30 秒现实 Timer、闪烁大图标、录音计时、“完成”和识别中提示。
- 完成 / 取消 / 挂起 / 关闭会话与场景退出时安全停止录音并清理 `user://voice_input/` 精确临时文件。

验收标准：

- 手动完成和 30 秒自动完成共用同一停止入口，不双发。
- 录音期间已有草稿不丢失；NPC-NPC 旁听模式不显示麦克风。
- 麦克风没有权限、设备不可用或录音为空时可恢复为手动输入。
- Music / SFX / Ambience / UI / Voice 音量不改变麦克风采集，玩家不会听到本地回放。

完成记录（2026-09-07）：

- 已开启 Godot 音频输入，并在静音 `MicRecord` 总线上配置唯一 `AudioEffectRecord`；`AudioStreamMicrophone` 只写入该总线，不进入本地监听。
- `Main/UI/DialogPanel` 已加入录音控制器、30 秒现实 Timer、输入框右侧本地 SVG 麦克风按钮，以及历史区中央的呼吸闪烁麦克风、计时、“完成”和识别中提示。
- 录音状态固定为 `IDLE / RECORDING / ANALYZING`。手动完成和 30 秒超时共用同一停止入口；会话替换、隐藏、取消、挂起、结束或场景退出会安全停止并只清理 `user://voice_input/voice_*.wav` 临时文件。
- 录音期间保留既有草稿；NPC-NPC 旁听隐藏入口；权限 / 设备 / 空录音失败恢复手动输入。T1601C 尚未接请求桥，因此本阶段完成录音后仅短暂展示识别态并明确提示下一阶段接入，不产生伪识别结果。
- `verify_t1601b_voice_recording_ui.gd`、既有对话 UI 与会话生命周期回归通过；Godot MCP 在正式 Main 读取到录音覆盖层、7/30 秒计时、草稿控件锁定和可用麦克风设备。未主动采集用户物理麦克风内容，实机权限 / 音质留给手动验收。

---

## T1601C Mock 语音闭环、文本拼装与追加

状态：Done
优先级：P2
前置任务：T1601A, T1601B
涉及文档：`VOICE_INPUT_AND_EMOTION.md`, `UI_UX.md`, `GODOT_ARCHITECTURE.md`, `AI_NPC_SYSTEM.md`, `MODULE_INDEX.md`

任务目标：

- 新增独立 Godot 语音请求桥，使用 multipart 把本次 WAV 发送到 `/voice/analyze`，不扩张现有文本 LLMBridge 职责。
- 对合法响应拼成 `transcript + （emotion_label）`；情绪缺失 / 非法时只保留 transcript。
- 将识别段追加到当前 `DialogInputEdit` 末尾，恢复光标与焦点，不自动发送。
- 使用 request_id + dialogue_id 拒绝会话结束后的迟到响应。

验收标准：

- 空草稿、非空草稿和末尾已有空白三类追加结果稳定；原草稿不被覆盖。
- 七类合法情绪映射正确；未知情绪不追加括号。
- Mock HTTP 错误、超时、非 JSON、空转写都保留草稿并恢复手动输入。
- 最终文字继续走现有 `DialogSystem.send_player_message(...)`，对话 Prompt / Schema / 事件逻辑不变。

当前进度（2026-09-07）：

- 新增独立 `VoiceInputBridge`，以 multipart 上传 WAV、request / NPC / dialogue id 和 `zh` locale；后端地址复用现有 LLMBridge 配置，但不复用其文本请求职责。
- 客户端校验 HTTP、JSON、fallback、request_id、dialogue_id、非空转写与七类情绪标签。合法情绪拼为 `转写（情绪地）`，未知 / 缺失 / 不匹配情绪只保留转写。
- `DialogPanel` 已按空草稿、普通结尾和空白结尾稳定追加，恢复光标与焦点且不自动发送；关闭 / 取消 / 挂起 / 切换会同时取消 HTTP、失效请求并清理 WAV。
- `verify_t1601c_voice_mock_roundtrip.gd` 对真实本地 Mock endpoint 完成三次 multipart 200 闭环，并覆盖七类情绪、未知情绪、HTTP 错误、超时、非 JSON、空转写、迟到响应、临时文件清理和“不自动发送”；T1601B、既有对话 UI 与会话生命周期回归通过。
- 首次 Godot MCP 验收时连接返回 `ECONNREFUSED 127.0.0.1:8765`；经用户授权重启 Godot 与 MCP broker 后恢复。正式 Main 运行态确认桥节点存在、`已有命令 保卫驿站。（愤怒地）` 正确追加、无自动发送、迟到结果被忽略、Recorder 回到 idle，编辑器日志无错误。
...
优先级：P2
前置任务：T1601C, T1402
涉及文档：`VOICE_INPUT_AND_EMOTION.md`, `API_BUDGET.md`, `AI_NPC_SYSTEM.md`, `PROMPTS.md`, `backend/README.md`, `MODULE_INDEX.md`

任务目标：

- 在独立 VoiceModelAdapter 中接入华北 2（北京）非实时 `qwen3-asr-flash`，后端以 Base64 WAV 调用百炼 OpenAI 兼容接口。
- 从响应正文读取 transcript，从 `choices[].message.annotations[].emotion` 读取原生情绪并归一化。
- 正式支持 `neutral / happy / sad / disgusted / angry / fearful / surprised` 七类；缺失或非法时只返回文字。
- 情绪只通过最终括号后缀进入现有 `speaker_text`，不新增或修改 NPC 对话 Prompt。

验收标准：

- 能识别并显示平静、愉快、悲伤、厌恶、愤怒、恐惧、惊讶七类原生情绪。
- 示例结果为“我需要你保卫驿站。（愤怒地）”，并作为普通玩家文字进入现有对话请求。
- 第一版不把悲伤映射为低沉，也不伪造急促或嘲讽；这三类如需支持另立后续任务。
- 情绪识别失败时忽略，不阻塞对话。
- `DASHSCOPE_API_KEY` 只在后端环境中；Godot 不直连百炼。
- 基础 Mock 测试通过后必须用真实 API Key 验证；失败时记录真实原因并忽略情绪，不得用 Mock 情绪伪装识别成功。

完成记录（2026-09-07）：

- 独立 `VoiceModelAdapter` 已支持 `VOICE_PROVIDER=qwen3_asr_flash`，将 PCM WAV 编为 Base64 Data URI，通过北京业务空间专属 OpenAI 兼容 `/chat/completions` 调用百炼；Key、Workspace ID 和地址只从后端环境读取。
- 从 `choices[].message.content` 读取转写，从 `annotations[].emotion` 读取原生情绪；七类映射全部通过，缺失 / 非法情绪只保留文字。401/403、429、5xx、超时、传输、非 JSON、空转写均有明确错误、HTTP / 异常 usage 和 `fallback_used=false`。
- 语音成功费用按秒写入现有共享 `LLMCostLedger`；发送前按 `VOICE_DAILY_BUDGET_REQUEST_RESERVE_CNY` 预留，超预算在调用 Provider 前返回 `voice_budget_exceeded`，不影响文字对话。
- `verify_t1602_qwen_voice_provider.py` 覆盖请求体、业务空间地址、七类情绪、降级、错误矩阵、预算前置拦截和按秒费用入账；既有 Voice Mock、Schema 与 Model Adapter 回归通过。最终真实 `/voice/analyze` 使用 2.554 秒系统 TTS 普通话 WAV 成功转写“我需要你保卫驿站。”，返回 `neutral / 平静地`、无 Mock fallback，并向共享账本写入估算费用 `0.0005618 元`。

---

## T1603 对话草稿无限暂存与 300 字发送校验

状态：Done
优先级：P2
前置任务：T1600
涉及文档：`VOICE_INPUT_AND_EMOTION.md`, `UI_UX.md`, `DATA_SCHEMA.md`, `GODOT_ARCHITECTURE.md`, `MODULE_INDEX.md`

任务目标：

- `DialogInputEdit` 不设置输入长度上限，不截断键盘或语音追加文本。
- 发送按钮与 Enter 共用发送前校验；最终去首尾空白文本超过 300 字时弹出“输入文字超过上限300字”。
- DialogPanel 做用户提示，DialogSystem 做第二层权威入口保护，两者读取同一配置。
- 修正现有先清空再提交的顺序，只有系统接受发送后才清空草稿。

验收标准：

- 299 / 300 字可发送，301 字不可发送；汉字、标点、内部空格和情绪后缀都计入。
- 超限时全文保留并恢复焦点，不追加历史、不发后端请求、不改变公开性或特殊 toggle。
- 语音追加可以让草稿超过上限，系统不截断；玩家编辑到合法长度后可正常发送。
- 发送按钮和 Enter 行为一致，其他调用者绕过 UI 时仍被 DialogSystem 拒绝。

完成记录（2026-09-07）：

- `DialogInputEdit.max_length=0` 保持无限暂存。发送按钮与 Enter 共用 `_send_current_text()`，按去首尾空白后的 `String.length()` 从统一配置读取 300 字上限。
- 301 字时显示独立 `AcceptDialog`：“输入文字超过上限300字”；全文、公开性、特殊 toggle、历史和请求状态保持，关闭弹窗后恢复输入焦点。只有 DialogSystem 接受发送后才清空草稿。
- `DialogSystem.send_player_message()` 在激活草稿或产生任何副作用前执行第二层校验，并返回 `input_too_long`、上限和实际字符数，阻止其他调用者绕过 UI。
- `verify_t1603_dialogue_input_limit.gd` 覆盖 299 / 300 / 301、汉字 / 标点 / 内部空格 / 情绪后缀、语音追加至 302 字、不截断、UI / 系统双层拦截和无传输副作用；语音 B/C、对话 UI、会话生命周期回归通过。Godot MCP 正式 Main 弹窗、草稿、焦点和无历史 / 等待变化验收通过，编辑器无错误。

---

## T1604 语音输入完整回归、真实 API 与前端验收

状态：Partial
优先级：P2
前置任务：T1601, T1602, T1603
涉及文档：`VOICE_INPUT_AND_EMOTION.md`, `CURRENT_STATE.md`, `TASKS.md`, `MODULE_INDEX.md`, `TECH_ARCHITECTURE.md`, `GODOT_ARCHITECTURE.md`, `DATA_SCHEMA.md`, `AI_NPC_SYSTEM.md`, `MEMORY_AND_INFO_SPACE.md`, `UI_UX.md`, `PROMPTS.md`, `API_BUDGET.md`, `DEV_LOG.md`, `backend/README.md`

任务目标：

- 完成后端 Schema / Mock / 错误矩阵、Godot 录音 / UI / 追加 / 会话生命周期、300 字边界与现有对话回归。
- 在用户授权且环境有真实 Key 后，以 `VOICE_PROVIDER=qwen3_asr_flash`、无 Mock fallback 完成国内网络真实调用。
- 使用 Main 前端验证按钮位置、闪烁提示、手动 / 30 秒完成、追加、不自动发送、错误恢复和超限弹窗。

验收标准：

- 普通话男女声、轻噪声、短句、接近 30 秒长句和七类情绪样本有可追踪结果。
- usage 能定位 request id、call_type、Provider、model、NPC / dialogue id、时长、估算人民币费用、HTTP / 异常和 `fallback_used=false`；不记录音频正文或 Key。
- 现有对话、应征、鼓励工作、鼓舞士气、策略、攻击、逃离挽留、挂起 / 恢复和 NPC-NPC 旁听回归通过。
- 功能可直接在 Main 前端触发和观察，不新增 GM 权威入口。
- 没有真实 Key 或真实百炼调用未通过时，T1602 / T1604 只能标记 Partial / Blocked，不得标 Done。

2026-09-08 验收进展：

- 后端 Schema / Mock / Provider / 预算专项通过；Godot 录音 UI、Mock 追加闭环、300 字边界、对话 UI / 会话生命周期、应征、工作鼓励 / 士气 / 策略、战时对话与 NPC-NPC 对话边界通过。
- 新增可复用的 Windows SAPI 中文样本生成、确定性轻噪声和真实矩阵工具。北京地域 `qwen3-asr-flash` 最终矩阵 11 / 11 成功，覆盖中文女声、轻噪、术语、七类目标语境和 26.955 秒长句，均 `fallback_used=false`；本次最终矩阵估算 `0.0143763 元`。模型实际把目标 happy 判为 neutral，其余目标语境分别得到 neutral / sad / disgusted / angry / fearful / surprised，按真实结果记录。
- Main MCP 正式运行态确认麦克风按钮位于输入行、图标 / Tooltip / 可见状态正确；MCP server `4.0.1` 与 addon `4.1.11` 版本不一致，冻结步进超时，但运行态脚本与节点读取仍可用。
- 保持 Partial：本机没有中文男声系统音源，未在未获单独隐私确认的情况下自动开启实体麦克风；中文男声、真实玩家七情绪表演、手动完成 / 实际等待 30 秒仍待玩家实机验收。既有 `verify_escape_intervention_dialogue.gd` 停在警示文字旧颜色断言，`verify_npc_npc_dialogue_observer_ui.gd` 停在投影鼠标点击旁听入口，均未归因为语音逻辑，需另行修复或更新基线后复验。
...
---
# T0390 失败结算统一 NPC 逃离状态

- 状态：Done（2026-09-08）
- 目标：主厅被摧毁导致失败时，结算快照中的全部 NPC 统一视为已经逃离驿站；保留各自真实入伍状态，并在生成结局前把统一逃离事实发送给 LLM。
- 验收：主厅失败、Mock 八人往返、模板降级、五波胜利、T0388 面板均通过；真实 `deepseek-v4-flash` 失败调用返回 8 个 `opening_status=escaped`，无 Mock fallback。
# T0391 失败撤离时机区分

- 状态：Done（2026-09-08）
- 目标：失败时区分 NPC 是失守前主动逃离，还是主厅失守后被迫撤离；差异同时进入结局面板与 LLM 权威事实。
- 验收：主厅失败专项构造神父提前离站，验证面板和快照为 1 个 `before_fall_voluntary`、7 个 `after_fall_forced`；后端端点、模板、胜利、Mock 往返通过；真实 DeepSeek 经一次纠错后正确保持两种叙事含义，无 fallback。
