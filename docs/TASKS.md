...
状态：Done（由 T0257 的伤员身体直达路线取代旧跨建筑语义路线并完成回归）
优先级：P1
...
修复第五波等正式战斗中敌人已经进入驿站后，部分已入伍持主武器 NPC 因自身仍在站外集结位、取马路线或其他地点而只有 `combat` 模式却拿不到站内目标、表现为不移动的问题。只要存在存活站内敌人，所有可行动的武装应征 NPC 都应从全站站内敌人域取得并保持一个合法目标；站内敌人清空后恢复普通 `37.2 m` 友军索敌域。
...
状态：Done（站外 `37.2 m` / 站内整站分域在场锁、异源实际伤害一次性最近重扫及移动 / 攻击共锁已成为正式合同）
优先级：P0
...
- 站外覆盖首次最近锁、在场保持、离圈重选与统一半径边界；站内覆盖跨普通半径的整站最近锁、在场保持和仅站内候选。
...
- `friendly_station_response_v2 / friendly_enemy_presence_lock_v1` 已接入正式友军行为模式、策略移动、攻击时间线和敌军对 NPC 的实际 HP 伤害入口；站外精确范围为 `37.2 m`，站内直接使用正式 `interior_polygon` 全域且排除站外敌人。
...
状态：Partial（功能、专项、视觉截图与关联回归完成；等待用户在 Main 确认废墟构图）
优先级：P0
...
状态：Doing（P0/P0R、P1–P7、P4R 与 P8A 实体功能灯已完成；P10A 音频基础接入进行中）
...
### T0135-P10A 音频总线、设置与统一播放入口（2026-09-01）
...
- 陨石冲击最多命中 12 人、燃烧不限该上限；弩床已改为 44 伤害 / 4.25 秒。工作流目标 0–1 发，虔诚流用约 166.67 人·小时争取五波各 1 发。
...
- 虔诚上限为 100；每名正在祈祷或主持弥撒的 NPC 每游戏小时产出 3 点。第 3–7 天每天 18:00 发生 5 波且包含前两天备战时，一次充能需要约 33.3 人·小时：从第 1–7 日日均 5 人祈祷 1 小时约提供 1 次，全员投入或额外排班可提高到约 2–3 次。
...
- 两名 NPC 各祈祷 1 游戏小时累计 6 点虔诚；暂停、赶路、中断后或满值时不继续改变可用值，施放后归零并可重新积累。
