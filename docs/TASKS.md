...
- 地点信息节点记录 `people_present`、建筑 HP、等级、可用状态、工作位/床位占用和当前公告/命令（公告只有广场/公告牌有）。
...
实现广场作为室外事件归属地和公共信息中枢的机制，并让广场公开事件即时广播给当时已经在广场的 NPC。后来进入广场的 NPC 只接收当前广场状态和公告牌当前内容，不继承过去事件。
...
- 调试触发公开事件后，广场节点能把事件广播给当前在场 NPC。
- 接收广播的 NPC 能把公开事件写入见闻库。
- NPC 后进入广场时不会获得过去的公开事件，只会获得当前状态快照和当前公告。
...
- 玩家对 NPC 的非对话交互写入目标 NPC 的 `event_log`，并按可见性通过地点/广场节点即时广播。
...
- NPC 在地点内接收到公开事件或状态变化广播后写入见闻库；进入地点只获得当前状态快照。
