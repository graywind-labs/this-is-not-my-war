...
给每个 NPC 建立当天短期记忆，用于后续 LLM 输入。

实现范围：

- NPC 自身事件写入短期记忆
- NPC 进入地点获得见闻
- 玩家对 NPC 的非对话交互写入短期记忆
- 可查看 NPC 当前短期记忆摘要
...
- NPC 工作、吃饭、睡觉、被给予金钱、被攻击等事件进入短期记忆。
- NPC 面板或调试工具能查看短期记忆。
- 每天结束时暂不清空，后续任务处理。
...
目标：玩家能够点击 NPC 对话，通过“提出应征”让 NPC 接受或拒绝；玩家可给钱、给装备、攻击，且这些行为进入记忆。
