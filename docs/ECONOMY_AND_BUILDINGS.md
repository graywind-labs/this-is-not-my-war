...
- 工作：调试接口 `debug_assign_work(npc_id, building_id)` 会安排 NPC 前往对应建筑，到达后按行动配置扣除输入资源、增加输出资源或恢复建筑 HP，并调整饱食度和疲劳度。当前已覆盖菜园产粮、食堂加工餐食、酒窖酿酒、铁匠铺制造武器/盔甲、工械坊制造工程器械、马厩产出马匹整备占位、围墙修补恢复 HP。
