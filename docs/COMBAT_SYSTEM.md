...
完整方案与受控结果见 `docs/COMBAT_PERFORMANCE_OPTIMIZATION_PLAN.md`。T0243 引导区没有容量限制；合法固定目标的“存在”不再为可达/不可达都返回 true 的结果穷举验路。正式引导区选择仍按原占用数、距离和 slot_id 排序并逐个验证可达性。占用查询仅在一次同步选点内共享实时 body 列表与 4m 网格；网格按最大 body 半径并额外扩一整格保守获取候选，最终仍作相同圆柱精确检查与名单排序。下一次调用立即重读位置/HP，不跨帧保存。

索敌仅读取 NPC 战斗身份/行为模式与建筑身份/HP 的最小投影；完整调试/UI 快照保留。敌人运动表现只读实时 radius/base_speed，不生成全量移动调试快照。敌人数、战斗/物理/AI 频率、射程、攻击时间线、碰撞、动画和权威伤害不变。

玩家精确 GM 第三波链路进一步保留两项等值减负：友方远程单位按原排序逐项作相同精确检查，在第一个合法攻击位后立即结束，不再构造不会被使用的后续结果；NPC 状态仍同步写入并广播，但 ActionSystem 在该 NPC 没有待执行动作/导航失败/治疗/建筑协作且未昏迷逃离时快速返回，建筑功能灯仅在位置/逃离/睡眠占用签名变化或时间/建筑变化时重算。事件次数、提交顺序、攻击时钟、昏迷/逃离/治疗清理均保持。

表现层的建筑静态批次与方向阴影 2 级联不进入 CombatSystem 权威。精确 GM 链路最终仍约 `37.45/33.92/54.67 ms`（均值/P50/P95），任务为 Partial；物理均值约 `17.82 ms`，因此禁止通过关闭 actor 碰撞、RVO、动画或运动来宣称修复。所有已验证方向和禁重复条件见性能方案第 11 节。

新增显式 `debug_start_performance_capture / debug_get_performance_capture`：默认不创建采样器，GM 可限时观测帧分位数与逻辑/表现/导航等成本。每项最多 3600 样本，溢出计数明确暴露；高频逐敌分项可能早于整段帧采样达到上限。分项存在嵌套，不可相加；Process monitor 不等于纯 GDScript。战斗初始化重置采样器。

## T0385 建筑受击碎屑坐标分流

城门、仓库、主厅的 HP 飘字继续使用建筑顶部唯一锚点；`structure_damaged` 同时携带独立 `impact_world_position`，真实碰撞点优先，无碰撞信息时回退到建筑地面入口。CombatVFX 的结构碎屑读取命中字段，不再从顶部飘字坐标二次上抬。`world_position` 仍供既有反馈 / 音频消费者使用，伤害、HP、摧毁、攻击位和美术包络均未改变。

## T0379-I 正式陨石美术升级

PietySystem 现使用共享 FormalMeteorArt，保留原虔诚、范围 / 伤害 / 燃烧 / 无友伤 / 建筑禁投 / 友军排出和 StaticBody 生命周期。下落依旧 2.8 秒现实表现时间并服从主动暂停；美术轨迹与认可版末段加速 / 连续落地一致。落地冲击与冷却走统一战斗表现秒，全部新粒子同步暂停。战斗结束清岩体，弹坑从落地起独立按 24 游戏小时淡化，12 小时 Shader 与碎片 Alpha 均为 0.5。旧燃烧区仅隐藏红色圆盘，保留火焰 / 灯光 / 权威 tick；选点圈、原强震与现有两段音频不变。生命周期 / 安全 / 虔诚伤害 / 音频回归和正式八图专项通过，试片演示时长未接入。

## T0381 陨石大招声音可闻性

陨石下落 / 冲击仍是位置化 World 声，但使用独立大招档：120m 内不因俯视相机高度明显压低，最远距离至少 1500m 且无远距低通；下落 +2 dB、落地 +3 dB。两条素材、完整播放、下落跟随、落地位置和暂停规则不变，音频不参与冲击 / 燃烧伤害。

## T0379 陨石候选美术试片边界

R1：试片下落 / 触地震屏已直接复用正式 CameraRig 和原配置（0.1 / 9、0.82 / 23，冲击 2 秒），不再使用首版缩小振幅的预览曲线；只增强候选尾焰与落地 VFX，正式震屏 / 数值源无修改。

MeteorArtTrial 是独立表现演示，复用正式范围和 2.8 秒下落参数，但不实例化 PietySystem / CombatSystem，不消费虔诚、不产生伤害、友军位移或正式事件。A 使用正式 MeteorPresentation，B 使用仅试片引用的 MeteorArtCandidate。试片 8 秒后清除岩体、再保留弹坑 4 秒并用 10 秒淡化，方便完整观察；这些演示时间不改变正式战斗结束清理岩体与 24 游戏小时弹坑淡化规则。现有战斗合同及音频同步均未由本任务修改。状态 Partial 待用户美术评价，正式集成与战斗压力不属于本轮验收。

## T0380 虔诚蓄满提示

共享虔诚只有在 PietySystem 的真实提交值从未满跨到 100 时才广播一次 `piety_ready`。弹窗属于只读表现：不会自动进入选点、消费虔诚、创建 pending 陨石或提交伤害；持续满值不重复，正式施放归零后再次充满可再次触发。T0382 期间被否决的 4 秒圣咏已停用，等待用户重选，不影响既有陨石合法落点、无友伤、下落、冲击与燃烧权威。

## T0376 警报、战局提示与核心战斗声可闻性

- `combat_alarm`、`battle_started`、`wave_cleared`、最终 `victory / failure` 是玩家层面的战局状态提示，统一由 Combat 总线的全局 `AudioStreamPlayer` 播放；不挂主厅、敌军或镜头位置，不做距离衰减。
- 挥剑 / 长杆、弓弩与塔防释放 / 破风、弹体命中、敌我受击接触与概率惨叫、昏迷倒地、马匹阵亡和木石结构受击仍从真实攻击者、命中点或受击对象位置播放。它们统一使用 `combat_priority` 3D 档：`unit_size=48m`、最远距离至少 480m，并关闭 3D 播放器默认远距低通；保留方向感，但常用俯视镜头不会因垂直距离迅速听不见。
- 这次只修表现混音，不修改 SFX 用户设置、Combat 总线电平、50% 惨叫概率、同帧并发保护、攻击 / 碰撞 / HP 权威或已确认资产。
- T0392 增加守备官攻击例外：普通友方受击惨叫仍为 50%，但 `actor_damaged.source_id=guard_officer` 时强制为 100%，并按 NPC 性别复用既有男女语音；命中声仍必播。该例外不修改伤害、HP 或对话事实。

## T0375 统一警报声音触发

- `CombatSystem.trigger_combat_alarm` 完成原有见闻、资格与集结提交后，额外发送一次只读 `combat_alarm` 音频事件；失败返回不发声，重复警报仍按每次成功操作播放一次。
- `CombatAudioController` 播放既定 `sfx_world_battle_alert_one_shot`。T0376 起按用户定义改为无特定位置的全局提示；声音不决定 NPC 是否听见、不修改 rally、索敌或战斗状态，且与敌人入场提示仍是两种独立声音。
...
- NPCSystem 在 HP 实际下降后发送我方受击，并携带 `became_unconscious`；HorseSystem 只在马匹正式死亡后发送阵亡，普通受伤无声。BuildingSystem / DefenseDeviceSystem 只在 HP 实际下降后发送结构受损，并携带 destroyed 和提交坐标。T0376 起最终胜利 / 失败沿既有 `game_over_changed` 全局播放，不再取主厅位置做衰减。
...
- 敌人首次真实进入场景后才切 `music_battle`；最后一名敌人自然死亡、GM 清场或恢复为空状态后回到当前昼夜播放池的下一首。来袭预告和单纯触发警铃不会提前切歌。
...
T0386 后五波固定在第 1 日 23:00、第 3 日 08:00、第 4 日 04:00、第 5 日 12:00、第 6 日 18:00 到达，敌军总量仍为 `8 / 16 / 24 / 36 / 48`，对应基准入伍 `2 / 3 / 5 / 6 / 7` 与器械 `1 / 1 / 2 / 3 / 4`。第五波工作流仍以 7 武器、8 防具、2 马、1 弩床 + 3 箭塔对 48 敌；弩床仍为 44 伤害、4.25 秒间隔。具体兵种、装备、人时与恢复账见 `docs/GAME_BALANCE.md`。
...
五波权威时点现为第 1 日 23:00、第 3 日 08:00、第 4 日 04:00、第 5 日 12:00、第 6 日 18:00；敌军 `8 / 16 / 24 / 36 / 48` 的构成和逐字段战斗值不变。HUD 在每波 T-3h、T-30min 各给一次前端提示并强调警报按钮，但不自动鸣警或集结。

玩家鸣响战斗警报或敌军实际生成时，CombatSystem 会先请求 DailyReflectionSystem 取消所有在途首次熟睡反思、释放对应 NPC 的深睡锁，并把该睡眠窗口恢复为可重试。此中断不应用伪造反思、不清空短期记忆；战后下一段合格睡眠再走原反思链。警报仍只集结已入伍、持主武器且可行动的人，敌军生成与伤害结算不受反思系统反向控制。
