...
补充：运行时已开始区分工作模式、集结模式、战斗模式和非战斗人员避战模式，并取消旧式“战斗触发时全员心理判定”的实现路径。集结 / 战斗 / 避战公开对话结构化战时意向，以及战斗模式中 HP 低于 30% 的自身心理判定仍是后续任务。
