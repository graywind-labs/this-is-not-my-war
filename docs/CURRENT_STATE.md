...
版本：`0.0.215-art-pipeline-foundation`

T0123 当前状态为 `Done`。用户已确认将 Quaternius 系列作为项目主要 3D 资产家族，美术升级被提升为当前 P0：目标是俯视角、玩具感、粗壮比例的低多边形中世纪驿站；镜头拉远时建筑显示完整屋顶，拉近时屋顶在配置阈值间逐渐透明以观察真实室内。`docs/ART_DIRECTION.md` 已固化建筑、角色、动画、战斗反馈、UI、环境和性能最低标准，`docs/ART_PIPELINE.md` 已固化免费 Standard 资产先行、许可审计、原始源文件隔离、GLB 导入、继承场景和垂直切片门槛。

T0124–T0136 已按资产获取与许可、导入基线、屋顶透明、真实室内导航与地点权威迁移、角色骨骼动画、铁匠铺垂直切片、8 名 NPC、全部建筑、城防、战斗 VFX / 布娃娃、正式 UI、环境灯光音效和最终整合顺序登记。当前只完成文档与目录基础，没有下载 Quaternius 二进制资产、没有安装 Blender、没有修改 `Main.tscn` 或运行时逻辑。现有玩家可见表现仍是基础几何建筑与胶囊 / 球体 NPC；现有地点进入也仍是抵达建筑入口后切换逻辑地点，T0127 前不得把“真实室内”描述为已实现。
...
MerchantSystem 从 data/merchant_defs.json 读取每日 10:00-16:00 到访时段和报价，随 TimeSystem 控制后门商人标记；MerchantPanel 只提交买卖请求，ResourceSystem 结算第纳尔、粮食、木材、石料、铁和酒，MemorySystem 记录到达、离开与成功交易事件
  ↓
DefenseDeviceSystem 从 `data/defense_device_defs.json` 读取同为 1 阶的弩床 / 箭塔，以及围墙 `1 / 2 / 2 / 3 / 3 / 4`、主厅 `1 / 1 / 2 / 2 / 3 / 4` 的六级通用槽曲线；Main 世界 `+` 和两座建筑的 BuildingPanel 只提交类型与槽位，系统按器械定义原子扣除具体库存、占槽并结算 HP、防御、穿透、攻速和有效射程。主厅槽应用 `2.0x` 射程倍率；两类器械都按战斗动作秒调用 CombatSystem 结算敌人伤害，Station/DefenseDevices 只生成带 ModelMount 的可替换低模表现
  ↓
