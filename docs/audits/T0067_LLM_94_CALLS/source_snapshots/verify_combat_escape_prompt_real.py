    ...
    *,
    ...
    payload = {
        ...
        "speaker_context": SpeakerContext(
            ...
            health_status="轻伤但清醒",
            state={"location_id": npc.state.current_location},
        ).model_dump(),
...
def _dialogue_result(call: VerifiedCall) -> dict[str, Any]:
    parsed = NPCDialogueResponse.model_validate(call.body)
    return {
        "intent": parsed.intent,
        "recruitment_result": parsed.recruitment_result,
        "wartime_reaction": parsed.wartime_reaction,
        "should_end_dialogue": parsed.should_end_dialogue,
        "emotion": parsed.emotion,
...
def main() -> None:
    ...
    dialogue_expectations = {
        ...
        "dialogue_avoid_accept": (
            ...
            and result["intent"] == "accept_recruitment"
        ),
        "dialogue_avoid_reject": (
            lambda result: result["wartime_reaction"] == "none"
            and result["recruitment_result"] == "reject"
            and result["intent"] == "reject_recruitment"
        ),
        "dialogue_escape_stay": (
            lambda result: result["intent"] == "stay_after_intervention"
            and result["wartime_reaction"] == "none"
            and result["recruitment_result"] == "none"
            and result["should_end_dialogue"] is True
        ),
        "dialogue_escape_leave": (
            lambda result: result["intent"] == "leave_after_intervention"
            and result["wartime_reaction"] == "none"
            and result["recruitment_result"] == "none"
        ),
