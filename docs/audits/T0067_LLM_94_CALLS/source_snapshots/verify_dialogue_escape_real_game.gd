...
func _run_escape_stay() -> Dictionary:
	...
	var matched := (
		str(dialogue.get("intent", "")) == "stay_after_intervention"
	...
	result.merge({
		"expected": "stay_after_intervention",
		"matched_expected": matched,
		"intent": str(dialogue.get("intent", "")),
		"behavior_mode_after": str(state.get("behavior_mode", "")),
...
func _run_escape_five_leave() -> Dictionary:
	...
	for round_number in range(1, 6):
		...
		var leave := str(dialogue.get("intent", "")) == "leave_after_intervention"
		all_real = all_real and round_real
		all_leave = all_leave and leave
		round_results.append({
			"round": round_number,
			"intent": str(dialogue.get("intent", "")),
			"should_end_dialogue": bool(dialogue.get("should_end_dialogue", false)),
	...
	var result := {
		...
		"npc_id": npc_id,
		"expected": "five consecutive leave_after_intervention",
		"matched_expected": matched,
