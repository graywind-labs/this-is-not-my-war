...
“世界与角色 → 战斗性能（只读，30 秒）”提供开始采样、查看结果、停止采样；等价命令为 `combat_perf start / snapshot / stop`。默认关闭，30 秒自动停止，最多保存每项 3600 个样本。结果包含帧时间 P50/P95/P99、长帧比例、引擎 process/physics/navigation、Draw Calls、敌人数、暂停状态及战斗逻辑/表现/弹体分项。分项可能嵌套，不能相加；引擎 Process 不是纯脚本自耗时。

GM 使用分行摘要显示帧均值/P95/P99、核心分项、人数/暂停/Draw Calls 与环境，避免完整 JSON 被旧 2200 字符截断；全部指标和原始样本可通过 CombatSystem debug API 获取。高频逐敌分项先达到 3600 时只代表该项的前段样本，结果以 `dropped_samples` 明确显示溢出，不能当作整段连续统计。

先用既有波次入口进入目标战斗，关闭其他弹窗后开始采样；采样不会刷敌、改变伤害、暂停或时间倍率。采样中可以关掉 GM，30 秒后重新打开查看结果；暂停占比非零或敌人数变化须在比较时注明。自动化完整 Main 对照使用 `tools/benchmark_t0396_main_combat.gd`，其禁用外部模型请求仅属于测试夹具。

玩家当前标准复现顺序固定为：打开 GM → 点击“一键征召&配装”使 8 人全部入伍 → 点击第三波生成入口 → 保持 GM 打开观察真实交战。若要采样，在敌军生成后立即点“开始采样”，至少等到出现攻击/伤害后再“停止采样”；记录画面分辨率、相机、敌人数和帧 P50/P95。该入口不会自动应用隐藏世界、关闭碰撞/避障或阴影实验；这些只存在于自动基准的诊断参数，不是玩家优化档。

## T0386 压缩波次验收入口

本任务无需新增 GM 按钮：正式 HUD、工械坊项目和资源条都可直接验证。沿用既有时间设置入口跳到每波 T-3h / T-30min 可观察一次性提示与警报强调；点击正式警报按钮验证主动集结和反思锁释放，沿用“跳到下一波”验证实际生成。GM 不直接修改预警去重、半成品阶段或睡眠反思结果。

## T0384 NPC 赠酒后指定饮酒目标同步

- 先在世界中点击 NPC，并在其人物面板赠酒；随后打开 GM 时，“常用 / 正式行动 / AI信息”三个 NPC 选择器都会对齐这个世界当前选中 NPC，避免正式行动仍静默指向默认人物。
- GM 打开后，三个页签仍可分别改选，按钮也仍只作用于本页所选 NPC；再次关闭并打开 GM 才会重新对齐世界当前选择。
- 在“正式行动”选择 `drink_wine | 饮酒` 并点击“指定行动”，应消费该 NPC 个人酒 1 份，事件库最后新增喝酒成功；酒确实为 0 时才记录酒不足失败。GM 不修改赠酒或饮酒的权威结算。

## T0380 虔诚蓄满提示验收

- 不新增按钮。点击既有“填满虔诚”后应出现“天火已可用”弹窗，并同步播放一次已选 05 的 5 秒圣咏；最后 1.5 秒自然渐出。点击“太好了”关闭，随后仍需玩家点击满值虔诚按钮并选择合法地面才会正式施放。
- 持续满值或再次执行无效填满不得重复提示；正式施放归零后再次“填满虔诚”应重新提示。若建筑完工或幼马命名窗正显示，本提示进入既有 FIFO；待 T0382 选定声音后，也只能在轮到本提示真正显示时播放。
- GM 入口只调用 PietySystem 的既有调试填满接口；弹窗与声音不消费虔诚、不生成陨石或伤害。

## T0381 陨石大招声音验收

- 沿用“填满虔诚”与正式选点。全站常用俯视高度及进一步拉远时，下落破风和落地轰鸣都应清晰可闻；仍可辨认落点方向，但不能像普通工作声一样快速衰减。
- 下落 / 落地分别增加 +2 / +3 dB，并使用 120m 参考距离、至少 1500m 最大距离、无远距低通的大招档。GM 不直接播放或放大声音。

## T0133 战斗美术与布娃娃验收

- “世界与角色”页新增“T0133 战斗美术验收”行；“播放 VFX 样例”会在主厅入口生成拖尾、投射物命中、人物受击血迹和结构碎屑，等价命令为 `t0133_combat_art demo`。该入口只调用只读表现控制器，不造成伤害或修改建筑。
- “VFX / 布娃娃快照”或 `t0133_combat_art snapshot` 显示 VFX 并发 / 跳过计数，以及友军与活动敌人的布娃娃字段。配合既有 `damage_npc <npc_id> <damage>` 走真实 HP → 昏迷链，观察物理倒地、冻结与 30% 复苏回收。
- 血迹开关位于正式设置→画面；关闭后已有血迹立即清理，受击与 HP 结算不变。

## T0353 音频混音验收

- 不新增 GM 音频按钮。直接使用正式“声音设置”检查七个滑条；六类音频可分别拉到不同值，关闭按钮应在窗口消失的同一点击中播放木质声且没有后续翻页声。
- 在白天 / 夜晚正式场景滚动滚轮拉近、拉远镜头检查主环境底噪：拉近增强、拉远减弱但不消失。鸟、虫、猫头鹰、河流和炉火继续在各自位置衰减。
- 需要快速切换昼夜时可复用既有时间调试入口；该入口只改变权威时间，WorldAudioController 被动响应，不直接触发或伪造声音。

## T0336 铁匠铺 / 工械坊完成产物验收

- “制造马匹”页新增两个独立验收行：“铁匠铺完成产物验收”下拉列出铁盔、铁护臂、长杆武器、铁护腿、剑盾、锁子甲；“工械坊完成产物验收”下拉列出弓、弩、弩床、箭塔。选项直接来自 CraftingSystem 正式配方目录，不在 GMPanel 写死另一份产物表。
- 点击“完成并暂存”调用 `CraftingSystem.debug_complete_product(building_id, recipe_id)`，跳过制造时间与材料成本，向对应建筑新增一件正式 pending 成品并更新“最新完成”指针；正式库存不会增加，仍须使用既有“收下”完成入库。
- 命令等价入口为 `craft_complete <blacksmith|workshop> <recipe_id>`。该入口只为逐项检查常驻托盘、成品模型、最新替换和收获清空，不负责正常生产周期、材料结算或事件事实。

## T0334 小马出生并命名按钮

- 打开“制造 / 马匹”，在马匹操作行下方的“小马出生验收”独立行点击“小马出生并命名”。旧长行末尾的“强制繁育”已被替换，不保留重复入口；命令行 `horse_birth` 仍等价可用。
- 点击后进入与自然繁育相同的正式命名窗，输入框预填当前未使用马匹模板的默认名，最多 5 个字。此时小马仅处于待命名事务，马匹数量、事件和正式世界实体均未增加。
- 点击“确定”后，HorseSystem 才把小马加入正式马库、占用已预留马槽并写出生事件；GM 马匹下拉框会自动出现新马。可继续用“马匹快照”或点击世界中的小马检查名字、`life_stage=foal`、毛色、HP、饱食度和 HorsePanel。
- 马槽满、模板耗尽或已有待命名事务时按钮显示 HorseSystem 原始失败结果，不绕过容量、模板、名字唯一性或事务锁。

## T0135-P10H 陨石音效验收

- 不新增绕过虔诚 / 冲击的音频按钮。复用既有“填满虔诚”后正式点击陨石落点：施放成功才开始下坠声，声音应随可见陨石逼近；真实落地、伤害与燃烧出现时从同一位置播放轰鸣。
- 暂停期间陨石与下坠声音都应停住，继续后从原进度恢复。落地时下坠素材自然尾音可以与轰鸣短暂重叠，这是用户确认的完整素材合同。
- 镜头远离落点会衰减。GMPanel 只准备虔诚并走正式施放，不直接播放声音、不伪造 impact。

## T0135-P10G UI 与世界交互音效验收

- 不新增绕过业务的音频按钮。直接使用正式设置 / 建筑 / NPC 等面板、toggle、特殊互动成功弹窗和铁匠铺 / 工械坊真实成品收取检查 UI 声；“太好了”关闭窗只应听到普通木质点击。
- 用既有正式行动让 NPC 穿过任一实际存在门叶的普通建筑门，或靠近 / 离开正后门，开关各应只响一次并随镜头距离衰减。门叶停住或同向运动不能连响。铁匠铺正面全敞开、没有门，因此格伦进出不播放开门声。
- 用既有行商正式抵达 / 离场入口观察马车；只有车体实际移动时播放并跟随位置，暂停、停靠和离场完成立即静音。GMPanel 不开门、不补发库存成功、不直接控制音频播放器。

## T0135-P10F NPC 回复情绪语气声验收

- 复用既有“NPC 对话情绪”九个预览按钮，不新增绕过情绪合同的音频按钮。选定 NPC 后点击任一情绪，应同时显示既有气泡并从该 NPC 所在位置播放对应性别语气声。
- 男性“中性”会在两条确认资产中随机，女性“中性”和其余男女情绪使用固定映射。移动镜头靠近 / 远离可检查 3D 衰减；音效滑杆会通过 `Voice → SFX` 调整响度。
- 快速情绪序列中，同一 NPC 的下一声会替换尚未结束的上一声，不应叠成多人声；选择不同 NPC 分别预览时可以独立发声。该入口不创建对话、模型调用、事件或记忆。

## T0324 Mock Provider 的完整 NPC-NPC 验收

- 将 `backend/.env` 设为 `LLM_PROVIDER=mock` 并重启后端，再在“正式行动”选择发起者、目标和开场内容，点击“真实找人对话”。按钮名称中的“真实”表示走正式世界移动和对话状态机，不表示必须消耗真实模型额度。
- 应观察到只有发起者走近；目标接受前保持原位置 / 行动。Mock 接受后双方进入 `talk_to_npc`，继续自动轮次，并依据 Mock 的 `should_end_dialogue` 自然结束和恢复行动。完整流程不需要另点 Mock 快捷按钮。
- 显式 `provider=mock / fallback=false` 合法；真实 provider 失败后的 `fallback=true` 仍必须显示失败，不能用 Mock 内容假装成功。

## T0321 重复波次重新接敌验收

- 复用“战斗 / 敌人 → 生成所选波次”，不新增写战斗状态的按钮。让艾达完成第一波并留在正门内，清波后应回到工作状态且旧目标为空；随后再次选择第 1 波生成敌人。
- 当第二波尚在墙外、但进入艾达的 `37.2m` 水平索敌圈时，她应重新显示接敌、取得新的 `combat_target_enemy_id`，沿正式路线走出迎击并产生真实伤害。敌人先命中她也不得让她原地无反应。
- 艾达在驿站内时，目标域是“整个驿站 + 本人 `37.2m` 圈”的并集。若同时存在较远站内敌人和较近的圈内站外敌人，应锁较近者；把该站外敌人移出圈后，较远站内敌人仍可被锁定。站外 NPC 在破防时继续跨距离响应站内敌人。可用既有 NPC 状态与友军目标快照观察 `include_station_enemies / station_enemy_only`，不需要 Mock 或 LLM。

## T0320 多人协助行走验收

- 复用“建筑”的升级 / 修复准备入口与“正式行动”的协助指派，不新增跳过结算的专用按钮。先启动城门升级，指派两名 NPC 并等他们到位，再指派第三人；第三人应真实走向右侧站内施工点，不会在接近门楼时中断。
- 第三人到位前只是 pending；到位后建筑状态的 helper count 应从 2 变为 3，速度倍率随之提高。同样可对受损建筑逐一添加 3 名修复者验证。
- 协助治疗仍最多 2 人；前两人会真实接近并开始治疗，第三人应立即显示容量失败，不应留下 pending 或半途移动。这一边界没有因本次修复而放宽。

## T0319 真实找人对话的会合表现

- 在“正式行动”选择发起 NPC 和对话目标后点击“真实找人对话”。目标 NPC 应留在原地继续当前行动，只有发起者沿正式路线走到目标附近；不得再看到目标被拉到地点锚点或瞬移到发起者面前。
- 发起者进入交谈范围后先播放邀请表现并等待目标判定。目标接受之前不抢占目标行动；接受后双方才进入 `talk_to_npc`、出现正式对话表现并开始轮流回复。拒绝或 provider 失败则不伪装为已经开聊。
- 自动化先使用 fake bridge 验证空间和交接；真实验收脚本带显式 `T0319_ALLOW_REAL_LLM=1` 闸门，并固定只运行邀请 + 首轮回复，不得用它启动完整 8 人计划。

## T0318 正式行动交互验收

- NPC 找人对话：打开“正式行动”，选择“行动 NPC”和“对话目标”，点击“真实找人对话”。发起者应先实体接近目标，到位后进入正式邀请 / 对话链。显式 Mock 可用于完整机制验收；真实 provider 若不可用，仍应看到真实 HTTP / provider 失败原因，不得接受 fallback Mock 伪成功。
- 目标正在桌边吃饭或使用紧凑工位时不需要先手动停止；发起者若已走到 `2.3m` 内但桌体阻止更靠近，会直接播放桌边邀请。只有路线真实失败后才启用该余量，普通开阔地仍使用原接近距离。
- NPC 找守备官对话：在同页选择“行动 NPC”，填写开场意图并点击“NPC 找守备官对话”。NPC 应出现问号 / 主动交涉动作；点击该 NPC 后打开既有守备官对话窗。此按钮复用正式主动交涉权威，不直接生成模型回复。
- 协助治疗：先在“常用”用伤害入口把目标打至昏迷并准备足够资金，再回“正式行动”点击刷新。治疗目标下拉会把昏迷者置顶并显示“可治疗：昏迷”；点击“协助治疗”后观察治疗者接近、到位后的包扎表现，再推进时间或使用倍速，目标应出现绿色 HP 恢复。选择健康目标、自己治疗自己或未知目标会直接显示失败原因。
- 若启动规划因 provider 失败而暂停，先用启动失败窗选择继续游戏再观察移动；失败 NPC 现在会回到 `idle` 并保留 `daily_plan_failed`，不会以陈旧 `planning_day` 阻塞正式行动。

## T0135-P10C 工作与日常行动音效验收

- 不新增音频旁路按钮。复用“正式行动”现有 NPC / 行动分配，实际走到工位并开始后才应渐入声音；仍在 `moving_to_*` 或 pending 时保持静音。中断、替换行动或最后一人离岗后渐出。
- 分别触发菜园、食堂加工、马厩、酿酒、打铁、工械坊、训练、诊所、祈祷 / 弥撒、修复 / 升级、进食和饮酒即可试听。弥撒应先完整播放约 5 秒钟声，再进入圣咏；饮酒同一次动作不重复触发；睡眠保持无声。
- 镜头移近 / 远离行动者或目标建筑检查距离衰减。马厩可用既有马匹状态 / 进出厩流程核对有马鼻息和无马干草两条素材；声音层不直接修改马匹或动作状态。

## T0135-P10B 音乐与环境声验收

- 不新增重复按钮。使用“时间 → 设置天 / 时 / 分 / 秒”检查 `05:00–08:00` 鸡鸣、`06:00–18:00` 白天环境与 `18:00–06:00` 夜间环境；镜头移近马厩 / 食堂、三处林缘 / 草地和西侧河道可验证方向与距离衰减。
- 使用“战斗 / 敌人 → 生成所选波次”后应从当前昼夜播放池切到战斗曲；“清空敌人”后恢复当前时段播放池的下一首。警铃但无敌人时不得提前切歌。
- 铁匠铺或食堂正式工作入口会沿用既有工位 / 行动权威点亮炉火并同步播放位置炉火声；中断、离岗或空置后渐出。GM 不直接创建音频状态，也不修改工位或敌人在场事实。
...
- 不新增按钮；使用“一键征召&配装”“所选波次动态群战”、NPC 状态和现有战斗快照即可自然验收。`friendly_station_response` 应显示 `friendly_station_response_runtime_v4 / friendly_enemy_presence_lock_v1 / 37.2 m`，`locks[]` 可查看每名武装 NPC 的 `scope / include_station_enemies / station_enemy_only / target_enemy_id / target_present / pending`。
- 正常时站外 NPC 使用 `unified_radius / 37.2m`；站内 NPC 使用 `entire_station_plus_unified_radius` 并从并集锁最近者。破防时站外响应者为 `station_breach_global`，站内响应者为 `station_breach_plus_unified_radius`。异源实际伤害后，请求可能只存在一个 AI 预算帧；可结合 `different_attacker_damage_*` metrics 与 `target_selection_reason=different_attacker_damage_nearest_enemy_reacquire` 观察。
...
- 正常验收可直接把时间设为 `10:00`：先看到马匹、前座车夫和装满木箱 / 桶 / 袋的长方形围板货车斗沿后门固定路线行进，抵达前交易不可用；抵达后车斗上方出现放大的无文字羊皮纸钱袋图标。把时间设为 `16:00`，已打开的交易面板应立即关闭，标识失效，马车反向驶出并在路线边缘释放。
...
- 该分组的直接 Mock 只显示后端返回，不写入对话事件、不修改入伍、策略或 buff 状态；正式开窗和 T0324 GM 找人按钮走 DialogSystem 完整链。`dialogue_mock` / `dialogue_recruit` / `dialogue_morale` / `dialogue_strategy` / `dialogue_work` 只用于开发和 Schema 验证，不作为真实 API 验收；显式 Mock 可验收正式机制，但真实 provider 失败仍不得用 fallback Mock 伪装成功。
...
- 打开 `GM → 常用 → 群像结局验收`，点击“一键胜利结算”可直接复用 CombatSystem 的正式第五波胜利快照；点击“一键失败结算”会先经 BuildingSystem 权威伤害把主厅降至 0，再复用正式主厅失败提交。两个按钮都保留当前 NPC 的入伍、HP、昏迷、逃离、位置、指令和记忆，不由 GMPanel 拼造人物结局事实。
- 状态行依次显示“生成中… / LLM完成｜Provider / model / 模板降级｜原因”。“刷新结局状态”会把完整技术快照写入 GM 结果区。命令等价入口为 `epilogue_victory`、`epilogue_failure`、`epilogue_status`。
- 一局只能提交一次结果；验收另一种结果请重新进入 Main。启动真实后端时观察 LLM 结果；后端未启动时应看到明确模板降级。结算页仍是正式 HUD，GM 不复制第二套故事界面。
- T0388 后，同一入口打开重制后的正式结局页：检查胜利金色 / 失败灰暗主题、大号胜负标题、守住波数、时间 / 结局标题 / 主题三字段，以及八张“左侧画像、右侧逃离 / 入伍状态与故事”的纵向卡片。技术 Provider / 降级原因仍只在本区状态行查看。
- 首轮视觉反馈后，正式结局页应居中占屏幕宽高各约 2/3；结局标题和主题保留实际内容，但界面不再出现“结局标题”“主题”两个字段名。
