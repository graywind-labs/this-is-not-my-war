...
- `location_context`：地点当前状态读取占位，当前包含建筑 id、名称、等级、HP、标签、工作位数量和公开备注；不包含地点历史事件。
...
NPC 进入地点时，系统应生成 `location_entered` 事件并记录进入时地点状态快照；地点节点随后向在场 NPC 广播当前状态。NPC 只接收当前状态和之后发生的公开广播，不会因为进入某建筑而继承该建筑过去发生的事件。对话全文也作为对话事件 `payload` 保存，供后续对话、计划和睡前总结引用。
