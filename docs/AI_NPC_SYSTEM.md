...
- 武装 NPC 的目标不是 LLM 判断，也不是日计划内容。CombatSystem 按 NPC 当前世界位置选择唯一分域：驿站外为精确 `37.2 m` 水平圈，驿站内为整个正式驿站多边形；首次锁最近目标并保持到目标失效或离开当前分域。
- 驿站破防继续唤醒全部武装应征 NPC；站外响应者即使因此进入 combat，也必须等敌人进入自己的 `37.2 m` 目标域，不能跨地图锁住站内敌人。
...
- T0198 起，站内武装 NPC 只选择整座驿站内敌人，站外武装 NPC 则按统一 `37.2 m` 圈取得目标；原普通 5 米索敌已被取代。已分配坐骑者进入战斗时仍须先完成指定马匹取马，上马前不会因全域响应提前攻击。
